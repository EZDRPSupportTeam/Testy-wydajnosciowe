...
import { jUnit, textSummary } from "./libs/k6-summary/index.js";
...
export function runtimeParams(defaultTimeout) {
  ...
  if (runParams.duration > 2) {
    ...
    runParams.duration_stages.push({
      duration: `${warmDuration}m`,
      target: runParams.usersCount,
    });
  }
  runParams.duration_stages.push({
    duration: `${runParams.duration}m`,
    target: runParams.usersCount,
  });
...
export function summaryExport(data, globalCache) {
  ...
  try {
    if (
      ...
      data.setup_data.config.ezdrp.API_VERSION = dashboardSuite
        .wersja()
        .body.replace(/\"/g, "");
    }
  } catch (e) {
