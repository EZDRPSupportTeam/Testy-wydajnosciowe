...
class _Data {
  ...
  setData(key, value, section) {
    if (section && section != "") {
      if (this._subSection && this._subSection != "") {
        if (
          this._dataSet[this._subSection] === undefined ||
          this._dataSet[this._subSection] == null
        ) {
          let tempDataObj = {};
          tempDataObj[section] = {};
          ...
          if (
            this._dataSet[this._subSection][section] === undefined ||
            this._dataSet[this._subSection][section] == null
          ) {
            let tempDataObj = {};
        ...
        if (
          this._dataSet[section] === undefined ||
          this._dataSet[section] == null
        ) {
          let tempDataObj = {};
          tempDataObj[key] = value;
...
class _Session {
  constructor(testInputData, globalCache, envKey) {
    this._config = new _Config(
      testInputData && testInputData.config ? testInputData.config : {},
      envKey,
    );
    this._data = new _Data(
      testInputData && testInputData.data ? testInputData.data : {},
      envKey,
    );
    this._params = new _Params(
      testInputData && testInputData.params ? testInputData.params : {},
      envKey,
    );
    this.envKey = envKey;
  ...
  sleepQuick(factory) {
    ...
    this.sleepMinMax(
      SLEEP_QUICK_MIN * localFactory,
      SLEEP_QUICK_MAX * localFactory,
    );
  }
  sleepLong(factory) {
    let localFactory = factory ? factory : 1;
  ...
  setStanowiskoByKey(key) {
    ...
    if (
      danePracownika &&
      danePracownika.uzytkownik.listaStanowisk &&
      danePracownika.uzytkownik.listaStanowisk.length > 1
    ) {
      ...
      if (userParamsData && userParamsData.stanowiska_map) {
        ...
        if (stanowiskoToSet) {
          ...
          log(
            `nie wybrano stanowiska ${key} ${this.getLogin()} ${JSON.stringify(
              userParamsData.stanowiska_map,
            )}`,
          );
        }
      } else {
