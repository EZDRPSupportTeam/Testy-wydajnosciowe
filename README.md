...
Repozytorium zawiera kod skryptu (opartym na narzędziu k6) pozwalający założyć nowy podmiot (dowolną liczbę podmiotów) który można wykorzystać do przeprowadzenia testów wydajnościowych. Podmiot zakładany jest według wzorca (szablonu) opisanego w pliku w formacie JSON: `params/input/kuip.json`.
...
Login zawiera także fragment oznaczenia podmiotu (tzw. numer podmiotu) tj. `abe5`, który wynika z zastosowanego w komendzie parametru `PRAND="5"`. Przykładowo zmiana parametru na `PRAND="123"` spowoduje utworzenie podmiotu (o numerze 123) którego loginy użytkowników będą miały postać: `u1abe123@ezdrp.gov.pl` (w przypadku pierwszego użytkownika).
