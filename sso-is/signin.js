...
class _SsoSignIn {
  ...
  _checkAuthStatus(idTokenHint) {
    ...
    let authCheckRes = this._oidc.checkAuthStatus(
      authCheckRedirectPath,
      authCheckState,
      idTokenHint,
    );

    let authCheckResUrlVerify = new URL(authCheckRes.url);
    let authCheckResUrlSearchVerify = new URLSearchParams(
      authCheckResUrlVerify.search.slice(1),
    );
  ...
  _getLoginPageSession() {
    ...
    let authLoginCookieUrl = new URL(
      `http://localhost${decodeURIComponent(cookiesForAuthLoginURL.login_sesion)}`,
    );
    let authLoginCookieUrlSearchVerify = new URLSearchParams(
      authLoginCookieUrl.search.slice(1),
    );
  ...
  _sendLoginPassword(user) {
    ...
    let authLoginPassResUrlSearchVerify = new URLSearchParams(
      authLoginPassResUrlVerify.search.slice(1),
    );
  ...
  _getUserToken(signInCode, isSignedIn) {
    ...
    let userToken = {
      ...
      refresh_token:
        token.refresh_token === undefined ? "" : token.refresh_token,
  ...
  _selectLoginAndPass() {
    let userParamsInput = {
      userId: this._session.getTestUserEngineId(),
      iterationId: this._session.getTestIterationEngineId(),
    };
      ...
      if (isSignedIn) {
        ...
        let user = userOverride
          ...
          : currentContext.login === ""
            ? this._selectLoginAndPass()
            : { login: currentContext.login };
        ...
        log(
          `relogin: ${currentContext.login} ${JSON.stringify(user)} ${JSON.stringify(userToken)} ${JSON.stringify(userData)}`,
        );
