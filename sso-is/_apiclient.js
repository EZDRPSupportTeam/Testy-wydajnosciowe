...
class _OidClient {
  ...
  checkAuthStatus(redirectPath, state, idTokenHint) {
    ...
    let pkceChallenge = b64_sha256(pkceCode)
      .replace(/=/g, "")
      .replace(/\//g, "_")
      .replace(/\+/g, "-");
    ...
    params.tags = {
      name: `${authorizeUrlObj.origin}${authorizeUrlObj.pathname}`,
    };
  ...
  getLoginPage(redirectPath, state) {
    ...
    let authorizeUrl = this.oidcConfig.getAuthUrl(
      signInParams.toString().replace(/\+/g, "%20"),
    );
    let params = {
      headers: {
        "Accept-Encoding": "gzip, deflate, br",
    ...
    if (
      res.cookies &&
      res.cookies["AntiForgeryCookie"] &&
      res.cookies["AntiForgeryCookie"].length > 0
    ) {
      this.oidcConfig.setAntiForgeryCookie(
        res.cookies["AntiForgeryCookie"][0].value,
      );
    }
    ssoMetricWaitingTime.add(res.timings.waiting);
    return res;
  ...
  endSession(redirectPath, idToken) {
    const endSessionParams = new URLSearchParams([
      [
        ...
        this.oidcConfig.getRedirectUrl(redirectPath),
      ],
