...
class _WebConfigLoader {
  loadConfig(sysConfigProvider, sessionConfig, ssoConfig) {
    ...
    ssoConfig[CONFIGKEY_CLIENTID] =
      `${appId}_${ssoConfig[CONFIGKEY_CLIENTDOMAINHASH]}`;
  }
}
class _Config {
  constructor() {
  ...
  getClientId() {
    return this._session.getConfig(
      CONFIGKEY_CLIENTID,
      oidcSessionConfigSection(),
    );
  }

  getRedirectUrl(urlPath) {
    let redirectUrl = this._session.getConfig(
      CONFIGKEY_REDIRECTURL,
      oidcSessionConfigSection(),
    );
  ...
  getEndSessionUrl(urlParams) {
    let endUrl = this._session.getConfig(
      CONFIGKEY_END,
      oidcSessionConfigSection(),
    );
