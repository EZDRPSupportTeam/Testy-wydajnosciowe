...
/**
 * Rejestracja RPW (Rejestr Poczty Wpływającej)
 * Kancelista rejestruje przesyłkę dla Kierownika
 */
function rejestracjaRPW(
  ezdRpAdmin,
  ezdRpKancelaria,
  ezdRpStruktura,
  session,
  ...
  if (isOdbiorcaKorespondencjiExists) {
    ...
    let wyszukaniOdbiorcy = ezdRpStruktura.getUsersExact(
      ezdRpStruktura.szukaj({ search: kierownikDoKorespondencjiNazwisko }),
    ...
    if (wyszukaniOdbiorcy && wyszukaniOdbiorcy.length > 0) {
      ...
      if (odbiorcaKorespondencji !== null) {
        ...
        let zarejestrujPrzesylkeParam = {
          ...
          czyFileMonitor: Math.random() < 0.5,
        };
        ...
        let zarejestrowanePrzesylki = ezdRpKancelaria.rejestrujPrzesylke(
          zarejestrujPrzesylkeParam,
        );

        if (zarejestrowanePrzesylki && zarejestrowanePrzesylki.errorId) {
          log(
            `nie zarejestrowano ${zarejestrujPrzesylkeParam.iloscRekordow} RPW do ${kierownikDoKorespondencjiNazwisko} przez ${currentUserLogin} ${zarejestrowanePrzesylki.errorId}`,
          );
        } else {
...
/**
 * Oznaczanie kopert korespondencji wychodzącej (KW)
 * Kancelista oznacza koperty jako wysłane
 */
function oznaczanieKW(ezdRpKancelaria, ezdRpProfile, session) {
  let korespondencjaWychodzaca =
    ezdRpKancelaria.zestawienieKorespondencjiWychodzacej({
      status: 4,
      dataWplywu: 2,
    });

  if (
    korespondencjaWychodzaca &&
    korespondencjaWychodzaca.items &&
    korespondencjaWychodzaca.items.length > 0
  ) {
    let przefiltrowaneItems = korespondencjaWychodzaca.items.filter(
      (item) =>
    ...
    for (let i = 0; i < limitKoperty; i++) {
      ...
      if (kopertaRKW && kopertaRKW.idRodzajPrzesylki) {
        let parametryCenyPrzesylki = {
          rodzajPrzesylki: kopertaRKW.idRodzajPrzesylki,
          cennikOperatora: kopertaRKW.idOperatorPrzesylki,
          strefaOperatora: kopertaRKW.idStrefaPrzesylki,
          priorytet: false,
        };

        let parametryRodzajePrzesylek = {
          operator: parametryCenyPrzesylki.cennikOperatora,
        };
        ezdRpProfile.pobierzRodzajePrzesylek(parametryRodzajePrzesylek);

        let cenyDostepne = ezdRpKancelaria.pobierzCenyPrzesylki(
          parametryCenyPrzesylki,
        );

        let strefyDostepne = ezdRpProfile.pobierzStrefyPrzesylek({
          operator: parametryCenyPrzesylki.cennikOperatora,
          rodzaj: parametryCenyPrzesylki.rodzajPrzesylki,
        });

        session.sleepQuick();

        let strefaDlaKoperty =
          strefyDostepne &&
          strefyDostepne.lista &&
          strefyDostepne.lista.length > 0
            ? randomFromArray(strefyDostepne.lista)
            : null;

        if (strefaDlaKoperty) {
          let priorytetyDostepne = ezdRpProfile.pobierzPriorytetyPrzesylek({
            operator: parametryCenyPrzesylki.cennikOperatora,
            rodzaj: parametryCenyPrzesylki.rodzajPrzesylki,
            strefa: strefaDlaKoperty.id,
          });

          let priorytetDlaKoperty =
            priorytetyDostepne && priorytetyDostepne.length > 0
              ? randomFromArray(priorytetyDostepne)
              : null;
          ...
          if (
            ...
            parametryCenyPrzesylki.priorytet = priorytetDlaKoperty;
            parametryCenyPrzesylki.strefaOperatora = strefaDlaKoperty.id;
            cenyDostepne = ezdRpKancelaria.pobierzCenyPrzesylki(
              parametryCenyPrzesylki,
            );
          }
          ...
          if (
            ...
            ezdRpKancelaria.pobierzCenyPrzesylki(parametryCenyPrzesylki);
            ezdRpProfile.pobierzRodzajePrzesylek(parametryRodzajePrzesylek);
            strefyDostepne = ezdRpProfile.pobierzStrefyPrzesylek({
              operator: parametryCenyPrzesylki.cennikOperatora,
              rodzaj: parametryCenyPrzesylki.rodzajPrzesylki,
            });
            priorytetyDostepne = ezdRpProfile.pobierzPriorytetyPrzesylek({
              operator: parametryCenyPrzesylki.cennikOperatora,
              rodzaj: parametryCenyPrzesylki.rodzajPrzesylki,
              strefa: strefaDlaKoperty.id,
            });
...
/**
 * Uproszczona funkcja testowa dla roli Kancelisty
 * - Wszystkie pauzy są krótkie (sleepQuick)
 * - Używa randomByProbability z konfigurowalnymi prawdopodobieństwami
 */
export function rolaKancelariaSimple(data, globalCache, session, ssoSignIn) {
  ...
  randomByProbability(
    () => {
      let pismaDoMetadanych = ezdRpPisma.pobierzListePism({
        mojePisma: true,
        brakMetadanych: true,
      });
      ...
      if (
        pismaDoMetadanych !== null &&
        pismaDoMetadanych.items !== undefined &&
        pismaDoMetadanych.items.length > 0
      ) {
        let liczbaPismdoUzupelnienia = Math.min(
          20,
          randomIntInclusive(1, pismaDoMetadanych.items.length),
        );
        uzupelnienieMetadanych(
          liczbaPismdoUzupelnienia,
          pismaDoMetadanych.items,
          ezdRpPisma,
          ezdRpKancelaria,
          ezdRpRejestry,
          ezdRpFiles,
        ...
        if (
          pismaDoMetadanych &&
          pismaDoMetadanych.errorId &&
          pismaDoMetadanych.errorId !== ""
        ) {
          log(
            `blad pobierania listy pism do rejestracji ${currentUserLogin} ${pismaDoMetadanych.errorId}`,
          );
        }
      }
    },
