...
export class ObslugaPrzestrzeni {
  ...
  otworzPrzestrzen() {
    ...
    this.przestrzenSzczegoly = noweZadaniaSuite.szczegolyPrzestrzeni({
      idZadanie: this.przestrzenId(),
    });
  }
  pobierzHistorie() {
    let noweZadaniaTests = this.suitesFactory.getNoweZadaniaTests();
  ...
  zmienTytulDokumentu() {
    ...
    if (akta && akta.length > 0) {
      ...
      const zmienNazwe = numerRpw
        ? noweZadaniaTests.zmienNazweDokumentuRpw({
            idDokumentPrzestrzeni: id,
            nazwa,
          })
        : noweZadaniaTests.zmienNazweDokumentu({
            idDokumentPrzestrzeni: id,
            nazwa,
          });
    }
  ...
  przekaz() {
    ...
    const noweZadaniaTests = this.suitesFactory.getNoweZadaniaTests();
    noweZadaniaTests.przekaz({
      idObieg: this.idObieg(),
      idObiekt: uzytkownik.idStanowiska,
      uwagi: `Proszę o  ${randomStringNumberCase(10)}`,
    });
  }
  dodajDokument() {
    const noweZadaniaSuite = this.suitesFactory.getNoweZadania();
    this.przestrzenSzczegoly = noweZadaniaSuite.dodajDokument(
      this.idPrzestrzenRobocza(),
      this.przestrzenId(),
    );
  }
  aktualizujDokument() {
