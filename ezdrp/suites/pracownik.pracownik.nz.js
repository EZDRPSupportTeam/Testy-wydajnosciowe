...
export function rolaPracownikNZGenerateData(
  data,
  globalCache,
  session,
  ssoSignIn,
...
export function rolaPracownikNZ(data, globalCache, session, ssoSignIn) {
  ...
  if (doObsluzenia && doObsluzenia.items && doObsluzenia.items.length > 0) {
    ...
    let przestrzenie = noweZadaniaSuite.wylosujPrzestrzenie(
      doObsluzenia,
      Math.min(limitRandom, doObsluzeniaRandom),
    );
  ...
  const listaSpraw = ezdRpSprawy.pobierzListeSpraw({
    rok: nowYear,
  ...
  if (listaSpraw && listaSpraw.items && listaSpraw.items.length > 0) {
    ...
    let sprawy = sprawySuites.wylosujSprawy(
      listaSpraw,
      Math.min(limitRandom, sprawyRandom),
    );
