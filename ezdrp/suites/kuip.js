...
const SLEEP_FACTORY =
  __ENV["SLEEP_FACTORY"] === undefined ||
  __ENV["SLEEP_FACTORY"] === "undefined" ||
  __ENV["SLEEP_FACTORY"] === ""
    ? 1
    : parseFloat(__ENV["SLEEP_FACTORY"]);
...
export class EzdRpTestSuiteKuip extends EzdRpTestSuite {
  ...
  listaUzytkownikow(podmiot) {
    ...
    if (podmiot && podmiot.id !== undefined && podmiot.id !== "") {
      let usersRes = ezdRpKuip.getUzytkownicy({
        idPodmiotu: podmiot.id,
        login: "",
      });
  ...
  zalozPodmioty() {
    ...
    podmiotyParams.forEachPodmiot((podmiot) => {
      ...
      let podmiotFound =
        podmiotRes && podmiotRes.items && podmiotRes.items.length > 0
          ? podmiotRes.items.find((p) => {
              return p.skrot && p.skrot === podmiot.symbol;
            })
          : null;
      if (podmiotFound != null) {
        let podmiotIdToGet = podmiotFound.id;
        log(
          `podmiot ${podmiot.symbol} found ${podmiotIdToGet} ${podmiot.czy_aktualizowac}`,
        );
        ...
        let zrodlaFinansowania = ezdRpKuip.pobierzZrodloFinansowania();
        log("zrodla finansowania");
        log(zrodlaFinansowania);

        if (
          !Array.isArray(zrodlaFinansowania) ||
          zrodlaFinansowania.length === 0
        ) {
          log("Brak danych źródeł finansowania lub nieprawidłowy format!");
          return; // Zakończ działanie funkcji/bloku kodu
        }

        log("zrodlo");
        log(zrodlaFinansowania[0]);
        let idZrodloFinansowania;

        if (
          zrodlaFinansowania[0] &&
          typeof zrodlaFinansowania[0] === "object"
        ) {
          idZrodloFinansowania = zrodlaFinansowania[0].id;
          log(
            "Pomyślnie pobrano ID źródła finansowania: " + idZrodloFinansowania,
          );
        } else {
          log(
            "Pierwszy element tablicy 'zrodlaFinansowania' jest undefined/null lub nie jest obiektem.",
          );
          return;
        }
        ...
        let danePodmiotu = {
          ...
          epuap: podmiot.epuap,
          rcl: podmiot.rcl,
          idZrodloFinansowania: idZrodloFinansowania,
        };
        let dodanyPodmiotRes = ezdRpKuip.dodajPodmiot(danePodmiotu);
        podmiotRes = ezdRpKuip.getPodmiot(dodanyPodmiotRes.id);
        if (podmiotRes && podmiotRes.id && podmiot.adres) {
          ezdRpKuip.aktualizujPodmiotAdres({
            id: dodanyPodmiotRes.id,
            nazwa: podmiot.nazwa,
            skrot: podmiot.symbol,
            typ: podmiot.typ,
            adres: podmiot.adres,
            idZrodloFinansowania: idZrodloFinansowania,
          });
    ...
    if (struktura.uzytkownicy && struktura.uzytkownicy.length > 0) {
      for (let i = 0; i < struktura.uzytkownicy.length; i++) {
        ...
        if (
          usersRes &&
          usersRes.items &&
          usersRes.items.some((item) => item.login === user.login)
        ) {
          userRes = usersRes.items.find((item) => item.login === user.login);
        } else {
          userRes = ezdRpKuip.utworzUzytkownika({
            imie: user.imie,
            nazwisko: user.nazwisko,
            login: user.login,
            idPodmiotu: idPodmiotu,
          });
          ezdRpKuip.zmianaHasla({
            idUzytkownika: userRes.id,
            haslo: user.haslo,
          });
        }
        if (!userRes) {
          usersRes = ezdRpKuip.getUzytkownicy({
            idPodmiotu: idPodmiotu,
            login: user.login,
          });
          if (
        ...
        if (
          ...
          user.stanowisko &&
          user.stanowisko != "" &&
          !podmiot.stanowiska.includes(user.stanowisko)
        ) {
  ...
  inicjalizacjaPodmiotu(idPodmiotu, numer) {
    log("inicjalizacja podmiotu id: " + idPodmiotu);
    ...
    if (currentPodmiotyList != null) {
      return ezdRpAdminPodmioty.inicjalizacjaPodmiotu({
        idPodmiotu: idPodmiotu,
        numer: numer,
      });
    }
  ...
  zalozSlownikStanowisk(podmiot) {
    ...
    podmiot.stanowiskaKuip =
      slownikStanowisk && slownikStanowisk.items ? slownikStanowisk.items : [];
  }
  zalozStrukturePodmiotu(podmiot) {
    ...
    if (
      !strukturaKuip ||
      !strukturaKuip.komorkiOrganizacyjne ||
      strukturaKuip.komorkiOrganizacyjne.length <= 0
    ) {
    ...
    this.zalozStrukturePodmiotuRecursive(
      podmiot,
      podmiot.struktura_kuip,
      podmiot,
      ezdRpKuip,
      strukturaKuip,
    );
  ...
  znajdzTypStruktury(nazwaTypu, listaTypow) {
    if (!listaTypow || listaTypow.length <= 0) {
      return null;
    }
    for (let i = 0; i < listaTypow.length; i++) {
      let t = listaTypow[i];
      if (t.nazwa.trim().toLowerCase() === nazwaTypu.trim().toLowerCase()) {
        return t;
      }
    }
    return null;
  ...
  znajdzUzytkownika(usrId, komorkaId, lista) {
    ...
    for (let i = 0; i < lista.length; i++) {
      let t = lista[i];
      if (t.idUzytkownik === usrId && t.idKomorkaOrganizacyjna === komorkaId) {
        return t;
      }
    }
    return null;
  ...
  zalozStrukturePodmiotuRecursive(
    podmiot,
    nadrzednyKuip,
    struktura,
    ezdRpKuip,
    ...
    let symbolToSearch =
      struktura.struktura_kuip &&
      struktura.struktura_kuip.symbol &&
      struktura.struktura_kuip.symbol != ""
        ? struktura.struktura_kuip.symbol
        : struktura.symbol;
    let kuipExisted = this.znajdzKomorke(
      symbolToSearch,
      strukturaKuip.komorkiOrganizacyjne,
    );
    ...
    if (kuipExisted) {
      ...
      let typStruktury = this.znajdzTypStruktury(
        struktura.typ,
        podmiot.typyKuip,
      );
      if (typStruktury == null) {
        ...
        kuipExisted = this.znajdzKomorke(
          struktura.symbol,
          aktualnaStrukturaKuip.komorkiOrganizacyjne,
        );
        if (kuipExisted) {
          struktura.struktura_kuip = kuipExisted;
          podmiot.struktura_kuip_list.push(kuipExisted);
          ...
          log(
            `nie znaleziono komórki ${struktura.symbol} dla ${struktura.nazwa} po dodaniu do struktury`,
          );
        }
      }
    }
    if (struktura.struktura_kuip) {
      if (struktura.uzytkownicy && struktura.uzytkownicy.length > 0) {
        for (let i = 0; i < struktura.uzytkownicy.length; i++) {
          ...
          if (usr.kuip) {
            let usrUzytkownik = this.znajdzUzytkownika(
              usr.kuip.id,
              struktura.struktura_kuip.id,
              aktualnaStrukturaKuip.stanowiska,
            );
            if (usrUzytkownik) {
              usr.stanowisko_kuip = usrUzytkownik;
            } else {
              let usrStanowisko = this.znajdzStanowisko(
                usr.stanowisko,
                podmiot.stanowiskaKuip,
              );
              if (usrStanowisko) {
                ...
                podmiot.isChanged = true;
                aktualnaStrukturaKuip = ezdRpKuip.getStruktura({
                  typ: "ROBOCZA",
                });
                usrUzytkownik = this.znajdzUzytkownika(
                  usr.kuip.id,
                  struktura.struktura_kuip.id,
                  aktualnaStrukturaKuip.stanowiska,
                );
                if (usrUzytkownik) {
                  ...
                  log(
                    `nie znaleziono stanowiska kuip dla uzytkownika ${usr.nazwisko} ${usr.imie} ${usr.stanowisko} po dodaniu do struktury`,
                  );
                }
              } else {
                log(
                  `nie znaleziono stanowiska kuip dla uzytkownika ${usr.nazwisko} ${usr.imie} ${usr.stanowisko}`,
                );
              }
            }
          } else {
      ...
      if (struktura.struktura && struktura.struktura.length > 0) {
        for (let i = 0; i < struktura.struktura.length; i++) {
          ...
          aktualnaStrukturaKuip = this.zalozStrukturePodmiotuRecursive(
            podmiot,
            struktura.struktura_kuip,
            str,
            ezdRpKuip,
            aktualnaStrukturaKuip,
          );
        }
      }
    }
  ...
  zalozStrukturePodmiotuOfflineRecursive(
    podmiot,
    nadrzednyKuip,
    struktura,
    ezdRpKuip,
    ...
    if (struktura.struktura_kuip) {
      ...
      if (struktura.struktura && struktura.struktura.length > 0) {
        for (let i = 0; i < struktura.struktura.length; i++) {
          let str = struktura.struktura[i];
          aktualnaStrukturaKuip = this.zalozStrukturePodmiotuOfflineRecursive(
            podmiot,
            struktura.struktura_kuip,
            str,
            ezdRpKuip,
  ...
  zaladujStrukturePodmiotuRecursive(
    podmiot,
    nadrzednyKuip,
    struktura,
    ezdRpKuip,
    ...
    if (struktura.struktura_kuip) {
      ...
      if (struktura.struktura && struktura.struktura.length > 0) {
        for (let i = 0; i < struktura.struktura.length; i++) {
          let str = struktura.struktura[i];
          aktualnaStrukturaKuip = this.zaladujStrukturePodmiotuRecursive(
            podmiot,
            struktura.struktura_kuip,
            str,
            ezdRpKuip,
  ...
  zalozSzablonyUprawnien(podmiot) {
    ...
    let checkTypExists = function (szablonNazwaToSearch, szablonyToCheck) {
      if (!szablonyToCheck || szablonyToCheck.length <= 0) {
        return null;
      }
      for (let n = 0; n < szablonyToCheck.length; n++) {
        let t = szablonyToCheck[n];
        if (
          t.nazwa.trim().toLowerCase() ===
          szablonNazwaToSearch.trim().toLowerCase()
        ) {
          return t;
        }
      }
    ...
    for (let i = 0; i < szablonyPodmiotuToBe.length; i++) {
      ...
      if (!rola.uprawnienia || rola.uprawnienia.length <= 0) {
        continue;
      }
      let typExisted = checkTypExists(rola.nazwa, szablonyUprawnienAsIs.items);
      if (typExisted == null) {
        typExisted = ezdRpAdmin.dodajSzablonUprawnien({
          nazwa: rola.nazwa,
          prawa: rola.uprawnienia,
        });
        ...
        if (typExisted) {
          ...
          log(
            `nie znaleziono szablonu uprawnien ${rola.nazwa} po dodaniu do listy`,
          );
        }
      } else {
        typExisted.prawa = null;
      ...
      if (root && root.uzytkownicy && root.uzytkownicy.length > 0) {
        for (let userFromRoot of root.uzytkownicy) {
          if (userFromRoot.stanowisko === "Administrator") {
            this.przypiszRoleUzytkownika(
              podmiot,
              root.symbol,
              ezdRpAdmin,
              userFromRoot.idStanowiska,
              {
  ...
  przypiszRoleUzytkownika(
    podmiot,
    symbol,
    ezdRpAdmin,
    idStanowiskoUprawnienia,
    usr,
  ) {
    let roleUzytkownikaAsIs = ezdRpAdmin.getUprawnieniaStanowiska({
      idStanowisko: idStanowiskoUprawnienia,
    });
    let roleUzytkownikaToBe = this.ustalRoleToBe(
      podmiot,
      usr.stanowisko,
      symbol,
      usr.random,
    ...
    if (roleUzytkownikaToBe && roleUzytkownikaToBe.length > 0) {
      ...
      for (let n = 0; n < roleUzytkownikaToBe.length; n++) {
        ...
        let rolaAsIsExisted = this.znajdzRoleKuip(
          rolaToCheck,
          roleUzytkownikaAsIs.items,
        );
        if (rolaAsIsExisted) {
        } else {
          ...
          if (rolaToBe) {
            ...
            log(
              `nie znaleziono roli ${rolaToCheck} w rola_kuip podmiotu ${podmiot.symbol}`,
            );
          }
        }
      }
      if (roleToAdd.length > 0) {
        ezdRpAdmin.dodajUprawnieniaStanowiska({
          idStanowisko: idStanowiskoUprawnienia,
          role: roleToAdd,
        });
        roleUzytkownikaAsIs = ezdRpAdmin.getUprawnieniaStanowiska({
          idStanowisko: idStanowiskoUprawnienia,
        });

        for (let n = 0; n < roleToAdd.length; n++) {
          let rolaToCheck = roleToAdd[n];
          let rolaAsIsExisted = this.znajdzRoleKuipById(
            rolaToCheck,
            roleUzytkownikaAsIs.items,
          );
          if (rolaAsIsExisted) {
            ...
            log(
              `nie znaleziono szbalonu uprawnienia ${rolaToCheck} u uzytkownika ${usr.nazwisko} ${usr.imie} po przypisaniu`,
            );
          }
        }
      } else {
      ...
      log(
        `brak danych stanowisko_kuip dla ${usr.nazwisko} ${usr.imie} ${usr.login} - nie nadano uprawnien`,
      );
    }
  }
    ...
    if (jrwaAll && jrwaAll.length > 0) {
      ...
      for (let j of jrwaAll) {
        ...
        if (jrwaForTyp && jrwaForTyp.length > 0) {
          let czySaPodteczki = jrwaForTyp.some(
            (item) => item.typ === "Podteczka",
          );
          for (let jt of jrwaForTyp) {
            if (jt.typ !== "Wykaz" || !jt.symbol.startsWith(j.jrwa)) {
              continue;
            }
            ...
            if (
              (j.nas !== undefined &&
                j.nas === true &&
                jt.nazwaNAS === undefined) ||
              jt.nazwaNAS === ""
            ) {
              ezdRpJrwa.ustawNAS({
                idJRWASchematWykaz: jt.id,
                czyNieStanowiAktSprawy: true,
                nazwaNAS: jt.nazwa,
              });
            }
              ...
              for (let jednostka_symbol of j.pt) {
                for (let dzial_podmiotu of podmiot.struktura_kuip_list) {
                  if (dzial_podmiotu.symbol === jednostka_symbol) {
                    ezdRpJrwa.dodajPodteczke({
                      idJednostka: dzial_podmiotu.id,
                      idSchematWykaz: jt.id,
                      nazwa: `podteczka autotestowa ${randomStringNumberCase(10)}`,
                      symbolJednostki: jednostka_symbol,
                      numer: 0,
                      rok: `${nowYear}`,
                    });
                    break;
  ...
  inicjalizacjaKreatoraPodmiotu(podmiot) {
    ...
    let ezdRpAdminPodmioty = new EzdRpAdminPodmioty(this.ezdRpClient);
    if (
      profilUzytkownika == null ||
      profilUzytkownika.uzytkownik === undefined ||
      profilUzytkownika.uzytkownik == null ||
      (profilUzytkownika &&
        profilUzytkownika.uzytkownik &&
        profilUzytkownika.uzytkownik.czyInicjalizacjaSystemuWymagana)
    ) {
  ...
  zalozKontakty(podmiot) {
    ...
    let typyPodmiotow = ezdRpProfile.pobierzWykazySlownika(
      "Adresaci.Podmiot.Typ",
    );
    let typPodmiotuJednostka = this.znajdzByKlucz(
      "Jednostka",
      typyPodmiotow.lista,
    );
    let typyAdresow = ezdRpProfile.pobierzWykazySlownika(
      "Adresaci.Adres.Typ_Jednostka",
    );
    let typAdresuKorespondencyjny = this.znajdzByKlucz(
      "AdresKorespondencyjny",
      typyAdresow.lista,
    );
    let typyMetadanych = ezdRpProfile.pobierzWykazySlownika(
      "EZDRP.Metadane.Klucze",
    );
    let kluczPodmiotu = this.znajdzByKlucz(
      "EZDRP.Metadane.Klucze.Adresat.Podmiot",
      typyMetadanych.lista,
    );
    let konfiguracjaMetadanych = ezdRpRejestry.getKonfiguracjaMetadanych({
      idObiekt: null,
      klucz: kluczPodmiotu.id,
    });
    let nipId = this.znajdzByKluczSystemowy(
      "EZDRP.Metadane.Nip",
      konfiguracjaMetadanych.listaKonfiguracji,
    );
    let regonId = this.znajdzByKluczSystemowy(
      "EZDRP.Metadane.Regon",
      konfiguracjaMetadanych.listaKonfiguracji,
    );
    let krsId = this.znajdzByKluczSystemowy(
      "EZDRP.Metadane.Krs",
      konfiguracjaMetadanych.listaKonfiguracji,
    );
    ...
    for (let i = 0; i < kontaktyPodmiotu.length; i++) {
      ...
      obecneKontakty = ezdRpRejestry.getAdresatow({
        search: newKontakt.nazwaPodmiotu.substring(
          0,
          newKontakt.nazwaPodmiotu.indexOf(" "),
        ),
      }).lista;
      let kontaktExisted = this.znajdzByNazwaPodmiotu(
        newKontakt.nazwaPodmiotu,
        obecneKontakty,
      );
      if (kontaktExisted) {
      } else {
        ...
        kontaktToAdd.listaAdresowEmail.forEach(function (k) {
          newKontakt.listaAdresowEmail.push({
            id: randomStringNumber(22),
            email: k.email,
            czyDomyslny: k.czyDomyslny,
          });
        });
        ...
        if (nipId) {
          newKontakt.metadaneAdresata.listaMetadanych[nipId.id] =
            kontaktToAdd.metadaneAdresata.listaMetadanych.NIP;
        }
        if (regonId) {
          newKontakt.metadaneAdresata.listaMetadanych[regonId.id] =
            kontaktToAdd.metadaneAdresata.listaMetadanych.REGON;
        }
        if (krsId) {
          newKontakt.metadaneAdresata.listaMetadanych[krsId.id] =
            kontaktToAdd.metadaneAdresata.listaMetadanych.KRS;
        }
  ...
  utworzCennik(podmiot) {
    ...
    let cennikExists = (cn) => {
      ...
      if (
        obecneCenniki == null ||
        obecneCenniki.items == undefined ||
        obecneCenniki.items == null ||
        obecneCenniki.items.lenght <= 0
      ) {
        return false;
      }
  ...
  zalozSklady(podmiot) {
    ...
    let ezdRpAdmin = new EzdRpAdmin(this.ezdRpClient);
    let lokalizacjaSkladu = {
      nazwa: "Skład kancelarii",
      adres: "Kancelaria Główna",
      symbol: "SKG",
    };
    ...
    if (
      obecneLokalizacje &&
      obecneLokalizacje.items &&
      obecneLokalizacje.items.length > 0
    ) {
    ...
    let lokalizacjaSkladuRes =
      ezdRpAdmin.dodajLokalizacjeSkladu(lokalizacjaSkladu);
    ...
    if (podmiot.idStanowiskaAll && podmiot.idStanowiskaAll.length > 0) {
      ezdRpAdmin.modyfikujLokalizacjeSkladu({
        id: lokalizacjaSkladuRes.id,
        nazwa: lokalizacjaSkladu.nazwa,
        adres: lokalizacjaSkladu.adres,
        ids: podmiot.idStanowiskaAll,
      });
    }
  }
  dodajKluczeAPI(podmiot) {
    ...
    let kluczExists = (kuid) => {
      if (
        obecneKlucze == null ||
        obecneKlucze.items == undefined ||
        obecneKlucze.items == null ||
        obecneKlucze.items.lenght <= 0
      ) {
        return false;
      }
    ...
    for (let i = 0; i < kluczePodmiotu.length; i++) {
      ...
      if (kluczExists(kluczUserToAdd.id)) {
        continue;
      }
      ezdRpAdmin.dodajKluczAPI({
        nazwa: `${kluczUserToAdd.login}`,
        klucz1: `${uuidv4()}`,
        idUzytkownikSystemowy: kluczUserToAdd.id,
      });
    }
  }
  zalozKartony(podmiot) {
    ...
    if (!skladyPodmiotu || skladyPodmiotu.length <= 0) {
      ...
      log("znaleziono sklady");
      let now = new Date();
      for (let i = 0; i < skladyPodmiotu.length; i++) {
        let skladToAdd = skladyPodmiotu[i];
        log(`skladToAdd ${skladToAdd.nazwa} id : ${skladToAdd.id}`);
        if (skladToAdd.id) {
          let rocznikSkladu = now.getFullYear();
          log(
            `otwieranie kartonu rocznik: ${rocznikSkladu} w skladzie: ${skladToAdd.id}`,
          );
          ezdRpKancelaria.otworzKarton({
            rocznik: rocznikSkladu,
            idSklad: skladToAdd.id,
          });
        }
      }
    }
  ...
  zaladujSzablonyDokumentow(podmiot) {
    ...
    if (
      !szablonyPodmiotyDoZalozenia ||
      szablonyPodmiotyDoZalozenia.length <= 0
    ) {
    } else {
      let ezdRpAdmin = new EzdRpAdmin(this.ezdRpClient);
      ...
      for (let i = 0; i < szablonyPodmiotyDoZalozenia.length; i++) {
        ...
        for (let j = 0; j < obecneSzablonyFoldery.katalogi.length; j++) {
          ...
          if (
            currentFolderIstniejacy.nazwa === szablonDoZalozeniaFolder.nazwa
          ) {
        ...
        if (szablonFolderIstniejacy === null) {
          let nowyFolder = ezdRpAdmin.dodajSzablonDokumentuFolder(
            szablonDoZalozeniaFolder,
          );
        ...
        let obecneSzablonyWybranegoFolderu =
          ezdRpAdmin.pobierzSzablonyDokumentowFoldery({
            idKatalogNadrzedny: idFolderuSzablonu,
          });
        for (
          let j = 0;
          j < obecneSzablonyWybranegoFolderu.szablony.length;
          j++
        ) {
          let currentSzablonIstniejacy =
            obecneSzablonyWybranegoFolderu.szablony[j];
          if (
            currentSzablonIstniejacy.nazwa === plikDoZaladowania.nazwaSzablonu
          ) {
        ...
        let fd = new FormData();
        let fileContent = encoding.b64decode(
          szablonyPodmiotyDoZalozenia[i].plikbase64,
        );
        let key = uploadParams.fileName;
        let fileContentType = key.endsWith("pdf")
          ? "application/pdf"
        ...
        obecneSzablonyWybranegoFolderu =
          ezdRpAdmin.pobierzSzablonyDokumentowFoldery({
            idKatalogNadrzedny: idFolderuSzablonu,
          });
        for (
        ...
        if (istniejacySzablonDokumentu && istniejacySzablonDokumentu !== null) {
          log(
            `Znaleziono szablon dokumentu ${plikDoZaladowania.nazwaSzablonu} ${istniejacySzablonDokumentu.id}`,
          );
          ...
          for (let stanowisko of podmiot.idStanowiskaAll) {
            dodanieDostepuParams.struktury.push({
              idObiekt: stanowisko,
              obiektTyp: "Stanowisko",
            });
          }
          ezdRpAdmin.dodajDostepDoSzablonowDokumentow(dodanieDostepuParams);
        } else {
          log(
            `nie znaleziono szablonu ${plikDoZaladowania.nazwaSzablonu} po dodaniu`,
          );
        }
      }
  ...
  zalozRejestry(podmiot) {
    ...
    if (!rejestryDoZalozenia || rejestryDoZalozenia.length <= 0) {
      ...
      let ezdRpRejestry = new EzdRpRejestry(this.ezdRpClient);
      let rejestryIstniejace = ezdRpRejestry.pobierzRejestry({
        pageInfo: { pageNumber: 0, pageSize: 20 },
        sort: [],
      });
      rejestryIstniejace = rejestryIstniejace.items
        ? rejestryIstniejace.items
        : [];
      for (let rejestrDoZalozenia of rejestryDoZalozenia) {
        if (
          !rejestryIstniejace.some((item) =>
            item.nazwa.startsWith(rejestrDoZalozenia.nazwa),
          )
        ) {
  ...
  konfiguracjaEpuap(podmiot) {
    ...
    if (
      podmiot.konfiguracja_epuap.idKonto &&
      podmiot.konfiguracja_epuap.idKonto === ""
    ) {
      return;
    }
  ...
  dodajPKN(podmiot) {
    ...
    if (obecnePkn.lista && obecnePkn.lista.length > 0) {
      ...
      ezdRpAdmin.dodaj_pkn({
        ...
        listaIdRodzajPrzesylki: [
          "EZDRP.KW.RodzajPrzesylki3",
          "EZDRP.KW.RodzajPrzesylki2",
        ],
        listaIdStrefaPrzesylki: [
          "EZDRP.KW.Strefa5",
          "EZDRP.KW.Strefa4",
          "EZDRP.KW.Strefa1",
        ],
  ...
  pobierzSid(podmiot) {
    let ezdRpProfile = new EzdRpProfile(this.ezdRpClient);
    ezdRpProfile.zaladujTozsamosc("", podmiot.symbol);
  }
}
...
export function zakladaniestruktury1(data, globalCache) {
  ...
  log(
    `zalogowano ${kuipSession.getLogin()} do KUIP przed zalozeniem podmiotow`,
  );
  ...
  log(`wylogowanie ${kuipSession.getLogin()} z KUIP po zalozeniu podmiotow`);
  ...
  ssoSignIn.signIn();
  log(
    `zalogowano ${session.getLogin()} do EZDRP przed inicjalizacja podmiotow`,
  );
  ...
  log(`wylogowanie ${session.getLogin()} z EZDRP po inicjalizacji podmiotow`);
  ssoSignIn.signOut();
  ...
  podmioty.forEachPodmiot((podmiot) => {
    let adminUser =
      podmiot.admins && podmiot.admins.length > 0 ? podmiot.admins[0] : null;
    if (adminUser) {
      data.data = {};
      data.params = {};
      globalCache = new Cache();
      ...
      log(`zakladanie typow komorek dla podmiotu: ${podmiot.nazwa}`);
      kuipSuite2.zalozTypyKomorek(podmiot);
      log(`zakladanie slownikow stanowisk dla podmiotu: ${podmiot.nazwa}`);
      kuipSuite2.zalozSlownikStanowisk(podmiot);
      log(`zakladanie struktury podmiotu dla podmiotu: ${podmiot.nazwa}`);
      kuipSuite2.zalozStrukturePodmiotu(podmiot);

      session.sleep(20);
      if (podmiot.isChanged) {
        log(`wdrazanie struktury dla podmiotu: ${podmiot.nazwa}`);
      ...
      suitesFactory = new EzdRpTestSuiteFactory(session);
      kuipSuite2 = suitesFactory.getKuip();
      log(`inicjalizowanie kreatora podmiotu dla podmiotu: ${podmiot.nazwa}`);
      kuipSuite2.inicjalizacjaKreatoraPodmiotu(podmiot);
      log(`zakladanie szablonow uprawnien dla podmiotu: ${podmiot.nazwa}`);
      kuipSuite2.zalozSzablonyUprawnien(podmiot);
      log(`przypisywanie uprawnien w podmiocie: ${podmiot.nazwa}`);
      kuipSuite2.przypiszUprawnienia(podmiot);
      log(`impotowanie JRWA w podmiocie: ${podmiot.nazwa}`);
      kuipSuite2.importJrwa(podmiot);
      log(`zakladanie kontaktow w podmiocie ${podmiot.nazwa}`);
      kuipSuite2.zalozKontakty(podmiot);
      log(`tworzenie cennikow w podmiocie ${podmiot.nazwa}`);
      kuipSuite2.utworzCennik(podmiot);
      log(`zakladanie skladow w podmiocie ${podmiot.nazwa}`);
      kuipSuite2.zalozSklady(podmiot);
      log(`dodawanie kluczy API w podmiocie ${podmiot.nazwa}`);
      kuipSuite2.dodajKluczeAPI(podmiot);
      log(`zakladanie szablonow dokumentow w podmiocie ${podmiot.nazwa}`);
      kuipSuite2.zaladujSzablonyDokumentow(podmiot);
      log(`zakladanie rejestrów w podmiocie ${podmiot.nazwa}`);
      kuipSuite2.zalozRejestry(podmiot);
      log(`konfigurowanie ePUAP w podmiocie ${podmiot.nazwa}`);
      kuipSuite2.konfiguracjaEpuap(podmiot);
      log(`dodawanie PKN w podmiocie ${podmiot.nazwa}`);
    ...
    let kancelariaUser =
      podmiot.kancelaria && podmiot.kancelaria.length > 0
        ? podmiot.kancelaria[0]
        : null;
    if (kancelariaUser) {
      data.data = {};
      data.params = {};
      ...
      ssoSignIn.signIn({
        login: kancelariaUser.login,
        password: kancelariaUser.haslo,
      });
      suitesFactory = new EzdRpTestSuiteFactory(session);
      kuipSuite2 = suitesFactory.getKuip();

      kuipSuite2.pobierzSid(podmiot);

      log(`zakladanie kartonow w podmiocie ${podmiot.nazwa}`);
      kuipSuite2.zalozKartony(podmiot);
      log(`wylogowanie ${kancelariaUser.login} z EZDRP`);
      ssoSignIn.signOut();
    } else {
      log(`no kancelaria for podmiot ${podmiot.nazwa}`);
    }
  });
  log(podmioty.podmioty);
}
