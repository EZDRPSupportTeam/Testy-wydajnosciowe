    ...
    zalozPodmioty() {
        ...
        podmiotyParams.forEachPodmiot((podmiot) => {
            ...
            if (podmiotFound != null) {
                ...
                if (!Array.isArray(zrodlaFinansowania) || zrodlaFinansowania.length === 0) {
                ...
                if (zrodlaFinansowania[0] && typeof zrodlaFinansowania[0] === 'object') {
                    idZrodloFinansowania = zrodlaFinansowania[0].id;
                    log("Pomyślnie pobrano ID źródła finansowania: " + idZrodloFinansowania);
                } else {
                    log("Pierwszy element tablicy 'zrodlaFinansowania' jest undefined/null lub nie jest obiektem.");
                    return;
                }


                let danePodmiotu = { nazwa: podmiot.nazwa, symbol: podmiot.symbol, typ: podmiot.typ, epuap: podmiot.epuap, rcl: podmiot.rcl, idZrodloFinansowania:idZrodloFinansowania };
        ...
        if (!skladyPodmiotu || skladyPodmiotu.length <= 0) {
            ...
            for (let i = 0; i < skladyPodmiotu.length; i++) {
                ...
                if (skladToAdd.id) {
                    ...
                    log(`otwieranie kartonu rocznik: ${rocznikSkladu} w skladzie: ${skladToAdd.id}`);
...
export function zakladaniestruktury1(data, globalCache) {
    ...
    log(`zalogowano ${kuipSession.getLogin()} do KUIP przed zalozeniem podmiotow`);
    ...
    ssoSignIn.signIn();
    log(`zalogowano ${session.getLogin()} do EZDRP przed inicjalizacja podmiotow`);
