...
export class EzdRpTestSuiteKuip extends EzdRpTestSuite {
    ...
    zalozTypyKomorek(podmiot) {
        ...
        let ezdRpKuip = new EzdRpKuip(this.ezdRpClient);
        let typyKomorek = ezdRpKuip.getTypyKomorek();
        let checkTypExists = function (typNazwaToSearch, typyToCheck) {
            if (!typyToCheck || typyToCheck.length <= 0) { return null; }
            for (let n = 0; n < typyToCheck.length; n++) {
                let t = typyToCheck[n];
                if (t.nazwa === typNazwaToSearch) { return t; }
            }
            return null;
        };
        for (let i = 0; i < typyPodmiotu.length; i++) {
            let typ = typyPodmiotu[i];
            let typExisted = checkTypExists(typ, typyKomorek.items);
            if (typExisted == null) {
    ...
    zalozSlownikStanowisk(podmiot) {
        ...
        let ezdRpKuip = new EzdRpKuip(this.ezdRpClient);
        let slownikStanowisk = ezdRpKuip.getSlownikStanowisk();
        let checkTypExists = function (stanowiskoNazwaToSearch, stanowiskaToCheck) {
            if (!stanowiskaToCheck || stanowiskaToCheck.length <= 0) { return null; }
            for (let n = 0; n < stanowiskaToCheck.length; n++) {
                let t = stanowiskaToCheck[n];
                if (t.nazwa === stanowiskoNazwaToSearch) { return t; }
            }
            return;
        };
        for (let i = 0; i < stanowiskaPodmiotu.length; i++) {
            let stanowisko = stanowiskaPodmiotu[i];
            let typExisted = checkTypExists(stanowisko, slownikStanowisk.items);
            if (typExisted == null) {
    ...
    zalozSzablonyUprawnien(podmiot) {
        ...
        let checkTypExists = function (szablonNazwaToSearch, szablonyToCheck) {
            ...
            for (let n = 0; n < szablonyToCheck.length; n++) {
                ...
                if (t.nazwa === szablonNazwaToSearch) { return t; }
            }
            return;
        };
    ...
    ustalRoleToBe(podmiot, stanowiskoNazwa, symbolKomorki, randomUzytkownika, login) {
        let nazwaToSearch1 = `${stanowiskoNazwa}`;
        let nazwaToSearch2 = `${symbolKomorki}_${stanowiskoNazwa}`;
        let nazwaToSearch3 = `${symbolKomorki}_${randomUzytkownika}`;
        let nazwaToSearch4 = login;
        let roleToBe = new Array();
        for (let i = 0; i < podmiot.stanowiska_map.length; i++) {
            let sm = podmiot.stanowiska_map[i];
            if (sm.nazwa === nazwaToSearch1 || sm.nazwa === nazwaToSearch2 || sm.nazwa === nazwaToSearch3 || sm.nazwa === nazwaToSearch4) {
                let roleToAdd = sm.role;
                if (roleToAdd && roleToAdd.length > 0) {
                    for (let n = 0; n < roleToAdd.length; n++) {
                        let rolaToAdd = roleToAdd[n];
                        if (!roleToBe.includes(rolaToAdd)) {
                            roleToBe.push(rolaToAdd);
                        }
                    }
                }
            }
