...
const apiMetricZadaniaSzczegoly = new Counter(
  "ezdrp_counter_zadania_szczegoly",
);
...
const apiMetricZadaniaPrzyjecia = new Counter(
  "ezdrp_counter_zadania_przyjecia",
);
const ezdrp_metric_zadanie_przyjecie = new Counter(
  "ezdrp_metrics_zadanie_przyjecie",
);
...
export class EzdRpTestSuiteBiurka extends EzdRpTestSuite {
  ...
  pobierzZadania(typ, typSzczegolowy) {
    ...
    ezdRpProfile.pobierzUstawienia([
      "EZDRP.System.PotwierdzeniePrawidlowejOperacji",
    ]);
  ...
  pobierzSzczegolyZadania(idZadania, czyPrzyjac) {
    ...
    ezdRpProfile.pobierzUstawienia(["EZDRP.System.RozmiarStrony"]);
    let aktaRes = ezdRpSprawy.pobierzAktaSprawy({
      idZadanie: idZadania,
      filterValue: "udostepnione",
    });
  ...
  przegladZadan(typLicznika, typZadania, sleepQuick, sleep, typSzczegolowy) {
    ...
    let licznikCount = dashboardSuite.pobierzLicznikByTyp(
      licznikiMenu,
      typLicznika,
    );
    if (licznikCount && licznikCount.value && licznikCount.value !== "0") {
      let lstZadania = typSzczegolowy
        ? this.pobierzZadania(typZadania, typSzczegolowy)
      ...
      if (lstZadania && lstZadania.length > 0) {
        ...
        for (let zi = 0; zi < maxIterations; zi++) {
          ...
          let szczegolyZadania = this.pobierzSzczegolyZadania(
            zadanieDoRealizacji.idZadanie,
            czyPrzyjac,
          );
          result.szczegoly++;
          if (sleepQuick) {
          ...
          if (!isToSkip) {
            if (zadanieDoRealizacji.typ === "Do podpisu") {
              ...
              zadanieDoRealizacji.typ === "Do akceptacji" ||
              zadanieDoRealizacji.typZadaniaEnum === 2
            ) {
              if (sleep) {
                sleep();
              }
              randomBool(
                () => {
                  if (
                    szczegolyZadania &&
                    szczegolyZadania.zadanie &&
                    szczegolyZadania.zadanie.dokumenty &&
                    szczegolyZadania.zadanie.dokumenty.length > 0
                  ) {
                    let filesTests = this.suitesFactory.getFilesTests();
                    let dokumentFile = filesTests.downloadPliku({
                      idDokument:
                        szczegolyZadania.zadanie.dokumenty[0].idDokument,
                    });
                  ...
                  if (
                    zadanieDoRealizacji.dokumenty &&
                    zadanieDoRealizacji.dokumenty.length > 0
                  ) {
                    let czyNowszaWersja =
                      ezdRpZadania.dokumentSprawdzCzyIstniejeNowszaWersja({
                        idDokumentWersja:
                          zadanieDoRealizacji.dokumenty[0].idDokumentWersja,
                      });
                    if (
                      czyNowszaWersja &&
                      czyNowszaWersja.lista &&
                      czyNowszaWersja.lista.length > 0 &&
                      czyNowszaWersja.lista[0].czyIstniejeNowszaWersja
                    ) {
                    } else {
              ...
              zadanieDoRealizacji.typ === "Do dekretacji" ||
              zadanieDoRealizacji.typZadaniaEnum === 4
            ) {
              if (sleep) {
                sleep();
              }
              ...
              if (
                ParamsUsers.isUserParamContainsRole(
                  this.session.getLogin(),
                  "Kierownik",
                )
              ) {
                let uzytkownicyDoWyboru = ParamsUsers.getUserNamesByRole(
                  "Pracownik merytoryczny",
                );
                let isUzytkownikDoWyboruExists =
                  uzytkownicyDoWyboru !== undefined &&
                  uzytkownicyDoWyboru !== null &&
                  uzytkownicyDoWyboru.length > 0;
                if (isUzytkownikDoWyboruExists) {
                  let ezdRpProfile = this.suitesFactory.getProfileTests();
                  ezdRpProfile.pobierzUstawienia([
                    "EZDRP.Zadania.MaksymalnaLiczbaOdbiorcowZadanUdostepnienia",
                  ]);
                  ...
                  let uzytkownikDoPrzekazania =
                    randomFromArray(uzytkownicyDoWyboru);
                  let wyszukaniOdbiorcy = ezdRpStruktura.getUsersExact(
                    ezdRpStruktura.szukaj({ search: uzytkownikDoPrzekazania }),
                    uzytkownikDoPrzekazania,
                  );
                  if (
                    wyszukaniOdbiorcy &&
                    wyszukaniOdbiorcy.length > 0 &&
                    szczegolyZadania.zadanie &&
                    szczegolyZadania.zadanie.dokumenty &&
                    szczegolyZadania.zadanie.dokumenty.length > 0
                  ) {
                    ...
                    let zadanieParam = {
                      idDokument:
                        szczegolyZadania.zadanie.dokumenty[0].idDokument,
                      nazwaDokument:
                        szczegolyZadania.zadanie.dokumenty[0].dokumentNazwa,
                      czyTylkoDoOdczytu:
                        szczegolyZadania.zadanie.dokumenty[0].czyTylkoDoOdczytu,
                      idPrzestrzen:
                        szczegolyZadania.zadanie.idPrzestrzenRobocza,
                    ...
                    if (
                      szczegolyZadania.zadanie.numerRPW !== undefined &&
                      szczegolyZadania.zadanie.numerRPW !== ""
                    ) {
                      zadanieParam.numerRPW = szczegolyZadania.zadanie.numerRPW;
                    }
                    let listaPism = this.pobierzPismaWtoku();
                    if (
                      !czyIstniejeZadanieDlaDokumentu(
                        listaPism,
                        szczegolyZadania.zadanie.dokumenty[0].idDokument,
                      )
                    ) {
                ...
                ezdRpZadania.cofnijZadanie(
                  zadanieDoRealizacji.idZadanie,
                  `zadanie Do-dekretacji nie u Kierownika ${this.session.getLogin()}`,
                );
                if (sleepQuick) {
                  sleepQuick();
                }
              ...
              zadanieDoRealizacji.typ === "Do realizacji" ||
              zadanieDoRealizacji.typZadaniaEnum === 1
            ) {
              if (
                ParamsUsers.isUserParamContainsRole(
                  this.session.getLogin(),
                ...
                ezdRpZadania.cofnijZadanie(
                  zadanieDoRealizacji.idZadanie,
                  `zadanie Do-realizacji nie u Pracownika ${this.session.getLogin()}`,
                );
                if (sleepQuick) {
                  sleepQuick();
                }
...
export function czyIstniejeZadanieDlaDokumentu(lista, idDokument) {
  if (!Array.isArray(lista)) {
    return false;
  }

  const element = lista.find((el) => el.idDokument === idDokument);

  if (!element) {
    return false; // nie znaleziono takiego elementu
  }

  return Array.isArray(element.listaZadan) && element.listaZadan.length > 0;
}
