...
export class EzdRpTestSuiteSprawy extends EzdRpTestSuite {
  nowaSprawaZGlownejStrony() {
    ...
    let jrwaSearchParam = {
      userId: this.session.getTestUserEngineId(),
      iterationId: this.session.getTestIterationEngineId(),
    ...
    let jrwaForSprawa = ParamsUsers.getUserJrwaFromWykaz(
      wykaz,
      jrwaSearchParam,
    );
    if (!jrwaForSprawa) {
  ...
  dodajZadanieAkceptacji(dokument, sprawa, userFilter) {
    ...
    randomBool(() => {
      ...
      let wyniki = ezdRpStruktura.getUsersExact(
        ezdRpStruktura.szukaj({ search: searchFilter }),
        searchFilter,
      );
      ...
      if (struktura.komorkiOrganizacyjne) {
        ...
        if (root.komorkiOrganizacyjne) {
          ...
          for (let i = 0; i < root.komorkiOrganizacyjne.length; i++) {
            ...
            let komorkaStruktura = ezdRpStruktura.pobierzDrzewo({
              id: komorka.id,
            });
    ...
    if (uzytkownicy.length <= 0) {
      log(
        `no users for akceptacja ${this.session.getLogin()} ${uzytkownicy.length}`,
      );
      return;
    }
    ...
    if (!userForAkceptuj || !userForAkceptuj.idStanowiska) {
      log(
        `no user selected for akceptuj ${this.session.getLogin()} ${uzytkownicy.length} ${userForAkceptuj ? userForAkceptuj.id : "no-id"}`,
      );
      return;
    }
    let ezdRpSprawy = new EzdRpSprawy(this.ezdRpClient);
  ...
  zaladujMetadane(idSprawy) {
    let ezdRpSprawy = new EzdRpSprawy(this.ezdRpClient);
    let aktaRes = ezdRpSprawy.pobierzMetadane({
      idObiekt: idSprawy,
      klucz: "EZDRP.Metadane.Klucze5",
    });
  ...
  zaladujSpraweAndAkta(idSprawy) {
    ...
    if (sprawaRes && sprawaRes.errorId) {
      log(
        `brak danych sprawy ${idSprawy} ${this.session.getLogin()} ${sprawaRes.errorId}`,
      );
    } else {
      let aktaRes = ezdRpSprawy.pobierzNoweAkta({
        idSprawa: sprawaRes.idSprawa,
        idPrzestrzenRobocza: sprawaRes.idPrzestrzenRobocza,
      });
  ...
  zmienTytul(sprawa, tytulOver) {
    ...
    let tytul =
      tytulOver && tytulOver !== ""
        ? tytulOver
        : `Zmieniona sprawa ${randomStringNumberCase(10)}`;
    let daneSprawy = {
      id: sprawa.idSprawa,
      tytul: tytul,
  ...
  dodajNotatkeWlasna(idPrzestrzen, idSprawy, nrSprawy) {
    ...
    ezdRpSprawy.dodajNotatkeWlasna(
      idPrzestrzen,
      nrSprawy,
      idSprawy,
      trescNotatki,
