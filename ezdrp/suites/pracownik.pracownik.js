...
export function rolaPracownikGenerateData(
  data,
  globalCache,
  session,
  ssoSignIn,
...
export function rolaPracownik(data, globalCache, session, ssoSignIn) {
  ...
  let biurkaSortByDataRejestracjiAsc = {
    sort: [{ propertyColumn: "dataRejestracji", propertySortType: "asc" }],
  };
  let biurko = biurkaSuite.pobierzSprawyWtoku(
    randomBool(null, null, 2) ? biurkaSortByDataRejestracjiAsc : null,
  );
  if (biurko && biurko.items && biurko.items.length > 0) {
    ...
    let sprawy = sprawySuites.wylosujSprawy(
      biurko,
      Math.min(limitRandom, biurkoRandom),
    );
  ...
  let biurkaSortByDataWplywuAsc = {
    sort: [{ propertyColumn: "dataWplywu", propertySortType: "asc" }],
  };
  biurko = biurkaSuite.pobierzPismaWtoku(
    randomBool(null, null, 2) ? biurkaSortByDataWplywuAsc : null,
  );
  if (biurko && biurko.items && biurko.items.length > 0) {
    ...
    if (liczbaPismDoZalozeniaSprawy > 0) {
      for (let i = 0; i < liczbaPismDoZalozeniaSprawy; i++) {
        biurko = biurkaSuite.pobierzPismaWtoku();
        let pismo =
          biurko && biurko.items && biurko.items.length > 0
            ? i >= biurko.items.length
              ? null
              : biurko.items[i]
            : null;
        if (pismo === null) {
        ...
        if (!pismo.atrybuty.czyUzupelnioneMetadane) {
          if (pismo.numer && pismo.numer !== "") {
            ...
            uzupelnienieMetadanych(
              ...
              ezdRpFiles,
              session,
              { uploadFiles: false, sklad: false },
            );
          } else {
            if (pismo.dokumentPisma && pismo.dokumentPisma !== "") {
              let idDokument = pismo.dokumentPisma.idDokument;
              let madataneConfig = ezdRpRejestry.getKonfiguracjaMetadanych({
                idObiekt: idDokument,
        ...
        ezdRpPisma.pobierzDokumenty({
          idPrzestrzen: pismo.idDokumentPrzestrzeni,
        });
        session.sleepQuick();
        randomBool(
          () => {
            let typProwadzenia = pismo.atrybuty.czyWSkladzieChronologicznym
              ? ParamsJrwa.typElektroniczna()
              : ParamsJrwa.typPapierowa();
            let jrwaSearchParam = {
              typ: typProwadzenia,
            ...
            while (!znalezioneJrwa && proba < 4) {
              ...
              if (jrwaForSprawa === undefined) {
                ...
                znalezioneJrwa = pomieszanaListaJrwa.find((element) => {
                  return (
                    element.symbol.startsWith(symbolJrwa) &&
                    (proba < 3
                      ? element.typProwadzenia == typProwadzenia
                      : true)
                  );
                });
              }
            ...
            if (!jrwaForSprawa || jrwaForSprawa.id === undefined) {
              ...
              let numerSprawyRes = ezdRpSprawy.pobierzNumerSprawy(
                pobierzNumerSprawyParams,
              );
              if (!numerSprawyRes || !numerSprawyRes.numerSprawy) {
                log(
                  `no numer sprawy for: ${pobierzNumerSprawyParams.symbol}.${pobierzNumerSprawyParams.rok}`,
                );
              } else {
                ...
                let znakSprawyRes =
                  ezdRpSprawy.pobierzZnakSprawy(znakSprawyParams);
                if (!znakSprawyRes || !znakSprawyRes.znakSprawy) {
                  log(
                    `no znak sprawy for: ${pobierzNumerSprawyParams.symbol}.${znakSprawyParams.numer}.${znakSprawyParams.rok}`,
                  );
                } else {
                  ...
                  if (!sprawaRes || !sprawaRes.idSprawa) {
                    let message =
                      sprawaRes && sprawaRes.messages
                        ? JSON.stringify(sprawaRes.messages)
                        : "";
                    log(
                      `no valid sprawa for: ${pobierzNumerSprawyParams.symbol}.${znakSprawyParams.numer}.${znakSprawyParams.rok} ${pismo.tytul} ${currentUserLogin}: ${message} ${pismo.atrybuty.czyUzupelnioneMetadane} ${jrwaSearchParam.typ} ${pismo.atrybuty.czyWSkladzieChronologicznym} ${pismo.id}`,
                    );
                  } else {
                  }
                }
