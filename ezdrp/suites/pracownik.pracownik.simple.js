...
/**
 * Uproszczona funkcja testowa dla roli Pracownika merytorycznego
 * - Wszystkie pauzy są krótkie (sleepQuick)
 * - Używa randomByProbability z konfigurowalnymi prawdopodobieństwami
 */
export function rolaPracownikSimple(data, globalCache, session, ssoSignIn) {
  ...
  let biurko = biurkaSuite.pobierzSprawyWtoku(
    Math.random() < 0.5 ? biurkaSortByDataRejestracjiAsc : null,
  );
  ...
  biurko = biurkaSuite.pobierzPismaWtoku(
    Math.random() < 0.5 ? biurkaSortByDataWplywuAsc : null,
  );
        ...
        randomByProbability(
          () => {
            ...
            if (!jrwaForSprawa || jrwaForSprawa.id === undefined) {
              log(`brak id dla JRWA ${currentUserLogin}`);
            } else {
                    ...
                    if (!sprawaRes || !sprawaRes.idSprawa) {
                      let message =
                        sprawaRes && sprawaRes.messages
                          ...
                          : "";
                      log(
                        `no valid sprawa for: ${pobierzNumerSprawyParams.symbol}.${znakSprawyParams.numer}.${znakSprawyParams.rok} ${pismo.tytul} ${currentUserLogin}: ${message}`,
                      );
                    }
                  }
                }
