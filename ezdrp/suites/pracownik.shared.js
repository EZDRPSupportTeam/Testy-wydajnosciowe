...
const ezdrp_metric_dodanieskanu_dokumentu = new Counter(
  "ezdrp_metrics_dodanieskanu_dokumentu",
);

export function uzupelnienieMetadanych(
  liczbaPismdoUzupelnienia,
  pismaDoMetadanych,
  ezdRpPisma,
  ezdRpKancelaria,
  ezdRpRejestry,
  ...
  for (let i = 0; i < liczbaPismdoUzupelnienia; i++) {
    ...
    if (
      pismoDoWyszukania.numer !== undefined &&
      pismoDoWyszukania.numer !== ""
    ) {
      let pismoZnalezione = ezdRpPisma.wyszukajPisma({
        searchValue: pismoDoWyszukania.numer,
      });
      if (pismoZnalezione == null || pismoZnalezione.idPismo === undefined) {
        continue;
      }
      let zalacznikiDoSkladu =
        ezdRpKancelaria.pobierzPismaDoRejestracjiWSkladzie({
          idPismo: pismoZnalezione.idPismo,
        });
      ezdRpRejestry.getKonfiguracjaMetadanych({
        klucz: "EZDRP.Metadane.Klucze2",
        idObiekt: pismoZnalezione.idPismo,
      });
      session.sleepLong();
      let zalacznikDoSkladu =
        zalacznikiDoSkladu.zalaczniki &&
        zalacznikiDoSkladu.zalaczniki.length &&
        zalacznikiDoSkladu.zalaczniki.length > 0
          ? zalacznikiDoSkladu.zalaczniki[0]
          : null;
      ...
      if ((templateConfig && templateConfig.uploadFiles) || !templateConfig) {
        if (zalacznikDoSkladu) {
          ...
          if (uploadedFile) {
            ...
            log(
              `brak informacji o pliku przy aktualizacji RPW ${currentUserLogin} ${pismoZnalezione.numerRPW}`,
            );
          }
        } else {
          log(
            `brak zalacznikow w RPW do aktualizacji skanu ${currentUserLogin} ${pismoZnalezione.numerRPW}`,
          );
        }
      }
      let adresaciMinisterstwa = ezdRpRejestry.wyszukajAdresataProponowanego({
        search: "mini",
      });
      if (
        adresaciMinisterstwa !== null &&
        adresaciMinisterstwa.lista !== undefined &&
        adresaciMinisterstwa.lista.length > 0
      ) {
        let adresatPisma = randomFromArray(adresaciMinisterstwa.lista);
        if (
          adresatPisma !== null &&
          adresatPisma.adresy !== undefined &&
          adresatPisma.adresy.length > 0
        ) {
          ...
          if (
            (templateConfig &&
              (templateConfig.sklad || templateConfig.sklad === undefined)) ||
            !templateConfig
          ) {
            ...
            if (!isPominacSklad) {
              ...
              if (
                zalacznikDoSkladu &&
                zalacznikDoSkladu.listaSkladow !== undefined &&
                zalacznikDoSkladu.listaSkladow.length > 0
              ) {
                let sklad = zalacznikDoSkladu.listaSkladow[0];
                if (
                  sklad.kartonSkladu !== undefined &&
                  sklad.kartonSkladu.id !== undefined &&
                  sklad.kartonSkladu.id !== ""
                ) {
                  ...
                  if (sklad.kartonSkladu.rocznik === rocznikSkladu) {
                    let rejestracjaWSkladzie = [
                      {
                        ...
                        idDokument: zalacznikDoSkladu.idDokument,
                        idPrzestrzenRobocza:
                          zalacznikDoSkladu.idPrzestrzenRobocza,
                    ...
                    ezdRpRejestry.zarejestrujZalacznikiWSkladzie(
                      rejestracjaWSkladzie,
                    );
                  } else {
                    log(
                      `nieaktualny rocznik ${rocznikSkladu} kartonu ${sklad.kartonSkladu.znak} dla skladu ${sklad.typSkladu.nazwa} do rejestracji w skladzie ${currentUserLogin} ${pismoZnalezione.numerRPW}`,
                    );
                  }
                } else {
                  log(
                    `brak kartonu dla skladu ${sklad.typSkladu.nazwa} do rejestracji w skladzie ${currentUserLogin} ${pismoZnalezione.numerRPW}`,
                  );
                }
              } else {
              }
          ...
          log(
            `brak adresow dla nadawcy w RPW ${currentUserLogin} ${adresatPisma.nazwaPodmiotu}`,
          );
        }
      } else {
      ...
      log(
        `pismo do uzupełnienia metadanych nie ma numeru ${currentUserLogin} ${pismoDoWyszukania.tytul} ${pismoDoWyszukania.id}`,
      );
    }
  }
}
export class ObslugaSprawy {
  constructor(sessionParam, sprawaParam) {
    ...
    this._operacje3 = [this.rejestracjaKorespondencjiWychodzacej];
  }
  sprawaId() {
    return this.sprawa.idSprawa;
  }
  sprawaZnak() {
    return this.sprawa.znak
      ? this.sprawa.znak
      : this.sprawaSzczegoly.getSprawa().znak;
  }
  sprawaIdPrzestrzen() {
    try {
      return this.sprawaSzczegoly
        ? this.sprawaSzczegoly.getSprawa().idPrzestrzenRobocza
        : "";
    } catch (e) {
      log(
        `id przestrzen undefined ${this.session.getLogin()} ${e} ${this.sprawaZnak()}`,
      );
    }
  }
  ...
  losujOperacje(minCount, maxCount, operacje, sleep) {
    let operationCount = randomIntInclusive(
      minCount,
      maxCount ? maxCount : operacje.length,
    );
  ...
  pobierzSklad() {
    let sprawySuites = this.suitesFactory.getSprawy();
    let sklad = sprawySuites.zaladujSklad(
      this.sprawaIdPrzestrzen(),
      this.sprawaId(),
    );
  }
  pobierzMetryke() {
    let sprawySuites = this.suitesFactory.getSprawy();
    let metryka = sprawySuites.zaladujMetryke(
      this.sprawaIdPrzestrzen(),
      this.sprawaId(),
    );
  }
  ...
  dodajFolder() {
    let sprawySuites = this.suitesFactory.getSprawy();
    this.sprawaSzczegoly = sprawySuites.dodajFolder(
      this.sprawaIdPrzestrzen(),
      this.sprawaId(),
    );
  }
  ...
  zmienTermin() {
    ...
    let termin = new Date(now);
    termin.setDate(termin.getDate() + randomIntInclusive(1, 3));
    this.sprawaSzczegoly = sprawySuites.zmienTermin(
      this.sprawa,
      termin.toISOString(),
    );
  }
  ...
  dodajZadanieAkceptacji() {
    ...
    if (anyDokument) {
      let uzytkownicyKierownicy = ParamsUsers.getUserNamesByRole("Kierownik");
      let isOdbiorcaKorespondencjiExists =
        uzytkownicyKierownicy !== undefined &&
        uzytkownicyKierownicy !== null &&
        uzytkownicyKierownicy.length > 0;
      if (isOdbiorcaKorespondencjiExists) {
        let kierownikDoKorespondencjiNazwisko = randomFromArray(
          uzytkownicyKierownicy,
        );
        let zlecenie = sprawySuites.dodajZadanieAkceptacji(
          anyDokument,
          this.sprawaSzczegoly.getSprawa(),
          kierownikDoKorespondencjiNazwisko,
        );
        if (zlecenie && zlecenie.commandResponseMessage) {
        }
      }
  ...
  dodajDokument() {
    let sprawySuites = this.suitesFactory.getSprawy();
    this.sprawaSzczegoly = sprawySuites.dodajDokument(
      this.sprawaIdPrzestrzen(),
      this.sprawaId(),
    );
  }
  ...
  dodajOpinie() {
    let sprawySuites = this.suitesFactory.getSprawy();
    let pismaSuite = this.suitesFactory.getPisma();
    this.sprawaSzczegoly = sprawySuites.dodajOpinie(
      this.sprawaIdPrzestrzen(),
      this.sprawaId(),
      this.sprawa.znak,
    );
  ...
  dodajNotatke() {
    let sprawySuites = this.suitesFactory.getSprawy();
    let pismaSuite = this.suitesFactory.getPisma();
    this.sprawaSzczegoly = sprawySuites.dodajNotatke(
      this.sprawaIdPrzestrzen(),
      this.sprawaId(),
      this.sprawa.znak,
    );
  ...
  dodajNotatkeWlasna() {
    let sprawySuites = this.suitesFactory.getSprawy();
    sprawySuites.dodajNotatkeWlasna(
      this.sprawaIdPrzestrzen(),
      this.sprawaId(),
      this.sprawa.znak,
    );
    ...
    if (anyDokument) {
      ...
      if (
        ...
        if (
          ...
          this.session.sleepQuick();
          let operatorzy =
            ezdRpProfile.pobierzWykazySlownika("EZDRP.KW.Operator");
          if (operatorzy && operatorzy.lista && operatorzy.lista.length > 0) {
            ...
            let rodzajePrzesylek =
              ezdRpProfile.pobierzRodzajePrzesylek(parametrySlownika);
            if (
              rodzajePrzesylek &&
              rodzajePrzesylek.lista &&
              rodzajePrzesylek.lista.length > 0
            ) {
              ...
              let strefyPrzesylek =
                ezdRpProfile.pobierzStrefyPrzesylek(parametrySlownika);
              if (
                strefyPrzesylek &&
                strefyPrzesylek.lista &&
                strefyPrzesylek.lista.length > 0
              ) {
                ...
                let priorytetyPrzesylek =
                  ezdRpProfile.pobierzPriorytetyPrzesylek(parametrySlownika);
                let rejestracjaKorespondencjiParam = {
                  ...
                  czyPriorytet:
                    priorytetyPrzesylek &&
                    priorytetyPrzesylek.length &&
                    randomBool()
                      ? priorytetyPrzesylek[0]
                      : false,
                ...
                let zarejestrowanaKorespondencja =
                  ezdRpKancelaria.rejestrujKorespondencjeWychodzaca(
                    rejestracjaKorespondencjiParam,
                  );
                if (
                  zarejestrowanaKorespondencja &&
                  zarejestrowanaKorespondencja.idGrupa
                ) {
                  this.session.sleepQuick();
                  let rodzajeWydrukow =
                    ezdRpKancelaria.pobierzSzablonyWydrukowKW({
                      czyZpo: false,
                      czyPriorytet: rejestracjaKorespondencjiParam.czyPriorytet,
                    });
                  this.session.sleepQuick();
                  let wydrukKw = ezdRpKancelaria.wybierzRodzajWydruku({
                    idGrupy: zarejestrowanaKorespondencja.idGrupa,
                  });
                  if (wydrukKw && wydrukKw.lista && wydrukKw.lista.length > 0) {
                    ...
                    log(
                      `nie wygenerowano wydruku KW ${zarejestrowanaKorespondencja.idGrupa} ${currentUserLogin}`,
                    );
                  }
                } else {
                ...
                log(
                  `brak stref przesylek dla operatora ${parametrySlownika.operator} i rodzaju ${parametrySlownika.rodzaj} w KW ${currentUserLogin}`,
                );
              }
            } else {
              log(
                `brak rodzajow przesylek dla operatora ${parametrySlownika.operator} w KW ${currentUserLogin}`,
              );
            }
          } else {
            log(
              `brak operatorów do wyboru w KW ${currentUserLogin} ${typeof operatorzy} ${typeof operatorzy.lista} ${JSON.stringify(operatorzy)}`,
            );
          }
        } else {
          log(
            `brak adresow dla nadawcy w KW ${currentUserLogin} ${adresatPisma.nazwaPodmiotu}`,
          );
        }
      } else {
