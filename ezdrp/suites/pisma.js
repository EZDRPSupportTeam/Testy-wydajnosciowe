...
export class EzdRpTestSuitePisma extends EzdRpTestSuite {
  nowePismoZGlownejStrony() {
    ...
    let dokument = ezdRpFiles.uploadPliku({
  ...
  nowePismoZGlownejStronyNZ() {
    ...
    let fileNameToUpload = fileParamsRes;
    let nowyObieg = ezdRpNoweZadania.dodajObieg({
      nazwaPliku: `${fileNameToUpload}`,
      isRpw: randomBool(),
    });
    ...
    let dokument = ezdRpFiles._fileUpload(
      fileToUpload,
      nowyObieg.tokenPrzesylaniaPlikow,
    );
    if (!dokument) {
      return;
    }
  ...
  pobierzDokumentPismaByBiurko(biurko, skip) {
    let ezdRpPisma = new EzdRpPisma(this.ezdRpClient);
    let pismoZalacznik = ezdRpPisma.szukajPismoZZalacznikiemByBiurko(
      biurko,
      skip,
    );
