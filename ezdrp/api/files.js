...
const apiMetricWaitingTime = new Trend(
  "ezdrp__uzyskaj_token_dodania_dokumentow",
);
const apiMetricWaitingTime2 = new Trend("ezdrp__dodaj_nowy_dokument");
const apiMetricWaitingTime3 = new Trend(
  "ezdrp__dokumenty_as_dodaj_nowa_wersje_dokumentu",
);
...
export class _EzdRpApiFiles extends EzdRpApi {
  ...
  akutalizujDokument(reqParams) {
    ...
    let reqData = this.toJson(reqDataRaw);
    let res = this.post(
      "/dokumenty-as-dodaj-nowa-wersje-dokumentu",
      reqData,
      params,
    );
  ...
  fileUpload(fileToUpload, fileToken) {
    ...
    params.headers["Authorization"] =
      `Bearer ${this.ezdRpClient.session.getSsoToken()}`;
    params.headers["ftoken"] = fileToken;
    params.headers["Content-Type"] =
      "multipart/form-data; boundary=" + fileToUpload.boundary;
    let reqData = fileToUpload.body;
    let res = http.post(
      `${this.ezdRpClient.ezdRPConfig.getFileUrl()}/Upload/file-upload`,
      reqData,
      params,
    );
    this.checkRes(res);
  ...
  directFileUploadToken(fileNameToUpload) {
    let params = this.getParams();
    params.headers["Accept-Encoding"] = "gzip, deflate, br";
    params.headers["Content-Type"] = "application/json";
    let reqDataRaw = {
      Files: [{ Name: fileNameToUpload }],
    };
    let reqData = this.toJson(reqDataRaw);
    let res = http.post(
      `${this.ezdRpClient.ezdRPConfig.getFileUrl()}/UploadToken/new-upload-token`,
      reqData,
      params,
    );
    this.checkRes(res);
  ...
  directFileChangeTitle(fileId, fileName, fileToken) {
    let params = this.getParams();
    params.headers["Accept-Encoding"] = "gzip, deflate, br";
    params.headers["headerToken"] = fileToken;
    params.headers["Content-Type"] = "application/json";
    let res = http.get(
      `${this.ezdRpClient.ezdRPConfig.getFileUrl()}/Download/TitleChange?idPlikStorage=${fileId}&name=${fileName}`,
      params,
    );
    this.checkRes(res);
    return res;
  ...
  directFileDelete(fileId, fileToken) {
    ...
    params.headers["Accept-Encoding"] = "gzip, deflate, br";
    params.headers["Content-Type"] = "application/json";
    let reqDataRaw = { idPlikStorage: fileId, headerToken: fileToken };
    let reqData = this.toJson(reqDataRaw);
    let res = http.post(
      `${this.ezdRpClient.ezdRPConfig.getFileUrl()}/Download/delete/file`,
      reqData,
      params,
    );
    this.checkRes(res);
  ...
  directFileGetDownloadToken(fileId) {
    ...
    params.headers["Content-Type"] = "application/json";
    let res = http.get(
      `${this.ezdRpClient.ezdRPConfig.getFileUrl()}/Token/gettoken/${fileId}`,
      params,
    );
    this.checkRes(res);
    return res;
  ...
  directDownloadFile(fileToken) {
    ...
    params.responseType = "none";
    let res = http.get(
      `${this.ezdRpClient.ezdRPConfig.getFileUrl()}/Download/file`,
      params,
    );
    this.checkRes(res);
    return res;
