...
export class _EzdRpApiAdminPodmioty extends EzdRpApi {
  pobierzPodmiotyKUIP_notused(param) {
    ...
    let reqDataRaw = {
      sort: [],
      pageInfo: { pageSize: 1000, pageNumber: 1 },
      filtersConjunction: [],
    };
  ...
  pobierzPodmioty(param) {
    ...
    let reqData = this.toJson(reqDataRaw);
    let res = this.post(
      "/ezdrp/administracja/kuip/podmioty/pobierz-wszystkie",
      reqData,
      params,
    );
    apiMetricWaitingTime.add(res.timings.waiting);
  ...
  inicjalizacjaPodmiotu(param) {
    ...
    let reqData = this.toJson(reqDataRaw);
    let res = this.post(
      "/ezdrp/administracja/podmiot/inicjalizacja",
      reqData,
      params,
    );
    apiMetricWaitingTime.add(res.timings.waiting);
  ...
  inicjalizacjaKreatoraPodmiotu() {
    ...
    let reqData = "";
    let res = this.post(
      "/ezdrp/administracja/podmiot/inicjalizacja/zapisz",
      reqData,
      params,
    );
    apiMetricWaitingTime.add(res.timings.waiting);
