...
const apiMetricWaitingTime7 = new Trend(
  "ezdrp__slownik_as_pobierz_wykazy_slownika",
);
...
export class _EzdRpApiProfile extends EzdRpApi {
  ...
  pobierzJednostkeNadrzedna() {
    ...
    let res = this.post(
      "/ezdrp/administracja/struktura-organizacyjna/jednostka/nadrzedna",
      reqData,
      params,
    );
  ...
  pobierzDostepneOperacje() {
    ...
    let reqData = this.toJson({});
    let res = this.post(
      "/ezdrp/administracja/menu/operacje/pobierz",
      reqData,
      params,
    );
  ...
  pobierzStatusPowiadomienia() {
    ...
    let reqData = this.toJson({});
    let res = this.post(
      this.drid("/pobierz-status-powiadomienia"),
      reqData,
      params,
    );
  ...
  pobierzTypyPrzesylek() {
    ...
    let reqData = this.toJson({});
    let res = this.post(
      "/ezdrp/korespondencja-wychodzaca/pobierz-typy-przesylek",
      reqData,
      params,
    );
  ...
  pobierzDoPowiadomienAkcjeWTle() {
    ...
    let reqData = this.toJson({});
    let res = this.post(
      "/ezdrp/administracja/akcje-w-tle/do-powiadomien/lista-stronnicowana",
      reqData,
      params,
    );
