...
const apiMetricWaitingTime2 = new Trend(
  "ezdrp__etykiety_utworz_folder_workspace",
);
...
const apiMetricWaitingTime7 = new Trend(
  "ezdrp__metadane_obiekt_pobierz_konfiguracje",
);
const apiMetricWaitingTime8 = new Trend(
  "ezdrp__ezdrp_administracja_sprawy_pobierz_roczniki",
);
...
export class _EzdRpApiSprawy extends EzdRpApi {
  dodajZadanie(param) {
      ...
      dokumenty: [
        {
          ...
          czyTylkoDoOdczytu:
            param.czyTylkoDoOdczytu !== undefined
              ? param.czyTylkoDoOdczytu
              : false,
        },
      ],
    ...
    let res = this.post(
      "/ezdrp/zadania/zadanie/zlec-do-dodania",
      reqData,
      params,
    );
  ...
  dodajFolder(reqParams) {
    ...
    let reqData = this.toJson(reqDataRaw);
    let res = this.post("/ezdrp/etykiety/folder/utworz", reqData, params);
  ...
  pobierzMetadane(reqParams) {
    ...
    let reqData = this.toJson(reqDataRaw);
    let res = this.post(
      "/metadane-obiekt-pobierz-konfiguracje",
      reqData,
      params,
    );
  ...
  pobierzRoczniki(reqParams) {
    let params = this.getParams();
    let reqData = reqParams ? this.toJson({}) : null;
    let res = this.post(
      "/ezdrp/administracja/ustawienia/sprawy/roczniki",
      reqData,
      params,
    );
  ...
  pobierzAktaSprawy(reqParams) {
    ...
    if (reqParams.filterValue && reqParams.filterValue != "") {
      filterValue = [
        {
          propertyValue: reqParams.filterValue,
          propertyFilterType: "",
          propertyColumn: reqParams.filterValue,
        },
      ];
    }
    let reqDataRaw = {
  ...
  pobierzListeSpraw(param) {
    ...
    let reqDataRaw = {
      ...
      sort: [{ propertyColumn: "dataZakonczenia", propertySortType: "desc" }],
    };
    let reqData = this.toJson(reqDataRaw);
