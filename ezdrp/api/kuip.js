...
export class _EzdRpApiKuip extends EzdRpApi {
  ...
  pobierzRejestrZmian(param) {
    ...
    let reqDataRaw = {
      idPodmiot: param.idPodmiotu,
      pageInfo: { pageSize: 10, pageIndex: 0, pageNumber: 1 },
      filtersConjunction: [],
      sort: [],
    };
    let reqData = this.toJson(reqDataRaw);
  ...
  pobierzRoleOrganizacyjne(param) {
    ...
    let reqDataRaw = {
      idPodmiot: param.idPodmiotu,
      pageInfo: { pageNumber: 0, pageSize: 10 },
      filtersConjunction: [],
      sort: [],
    };
    let reqData = this.toJson(reqDataRaw);
  ...
  getUzytkownik() {
    let params = this.getParams();
    let reqData = null;
    let res = this.post("/kuip/get-uzytkownik", reqData, params);
    apiMetricWaitingTime.add(res.timings.waiting);
    return res;
  }
  getPodmioty(param) {
  ...
  szukajPodmioty(param) {
    ...
    if (param && param.value !== undefined && param.value !== "") {
      reqDataRaw["filtersConjunction"] = [
        {
          propertyColumn:
            param.column && param.column !== "" ? param.column : "skrot",
          propertyFilterType: "ILIKE",
          propertyValue: param.value,
          propertyValueType: "string",
        },
  ...
  dodajPodmiot(param) {
    ...
    let reqDataRaw = {
      podmiot: {
        ...
        nazwaRCL: param.rcl,
        idZrodloFinansowania: param.idZrodloFinansowania,
      },
    };
    let reqData = this.toJson(reqDataRaw);
  ...
  aktualizujPodmiotAdres(param) {
    ...
    let reqDataRaw = {
      podmiot: {
        ...
        typ: param.typ,
        adres: {
          miejscowosc: param.adres.miejscowosc,
          kodPocztowyFormatted: param.adres.kodPocztowy,
          kodPocztowy: param.adres.kodPocztowy,
          ulica: param.adres.ulica,
          nrBudynku: param.adres.nrBudynku,
          nrLokalu: param.adres.nrLokalu,
        },
        idZrodloFinansowania: param.idZrodloFinansowania,
      },
    };
    let reqData = this.toJson(reqDataRaw);
    let res = this.post("/kuip/podmiot-aktualizacja", reqData, params);
    apiMetricWaitingTime.add(res.timings.waiting);
    return res;
  }
  pobierzZrodloFinansowania() {
    log("pobierz zrodlo finansowania");
    let params = this.getParams();
    let reqData = this.toJson();
    let res = this.post("/pobierz-zrodla-finansowania", reqData, params);
    apiMetricWaitingTime.add(res.timings.waiting);
    return res;
  }
  getPodmiot(param) {
  ...
  ustawAdministratora(param) {
    ...
    let reqData = this.toJson(reqDataRaw);
    let res = this.post(
      "/kuip/podmiot-aktualizacja-administratorow",
      reqData,
      params,
    );
    apiMetricWaitingTime.add(res.timings.waiting);
  ...
  getTypyKomorek() {
    ...
    let reqData = this.toJson(reqDataRaw);
    let res = this.post(
      "/pobierz-typy-komorek-organizacyjnych-widok",
      reqData,
      params,
    );
    apiMetricWaitingTime.add(res.timings.waiting);
  ...
  getStruktura(param) {
    ...
    let reqData = this.toJson(reqDataRaw);
    let res = this.post(
      "/pobierz-struktura-organizacyjna-robocza",
      reqData,
      params,
    );
    apiMetricWaitingTime.add(res.timings.waiting);
  ...
  inicjalizowanieStruktury() {
    ...
    let reqData = this.toJson(reqDataRaw);
    let res = this.post(
      "/przygotuj-plan-struktury-organizacyjnej",
      reqData,
      params,
    );
    apiMetricWaitingTime.add(res.timings.waiting);
  ...
  dodanieKomorkiOrganizacyjnej(param) {
    ...
    let reqDataRaw = {
      komorkaOrganizacyjna: {
        ...
        TypKomorkiOrganizacyjnej: param.TypKomorkiOrganizacyjnej,
      },
  ...
  wdrozenieStruktury() {
    ...
    let reqData = this.toJson(reqDataRaw);
    let res = this.post(
      "/wdroz-planowana-struktura-organizacyjna",
      reqData,
      params,
    );
    apiMetricWaitingTime.add(res.timings.waiting);
  ...
  zmianaHasla(param) {
    let params = this.getParams();
    let reqDataRaw = {
      IdUzytkownika: param.idUzytkownika,
      NoweHaslo: param.haslo,
      StareHaslo: "asd",
    };
    let reqData = this.toJson(reqDataRaw);
