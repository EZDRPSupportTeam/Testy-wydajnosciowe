...
class _WebConfigLoader {
  ...
  loadConfig(sysConfigSource, ezdRpConfigSet) {
    ...
    if (
      ezdRpConfigRes &&
      (ezdRpConfigRes.status == undefined || ezdRpConfigRes.status !== 200)
    ) {
      log(
        `get config failed: ${baseUrl}${baseConfigUrl}  RES: ${JSON.stringify(ezdRpConfigRes)}`,
      );
    }
    let ezdRpConfigParser =
      ezdRpConfigRes && ezdRpConfigRes.body
        ? new _WebConfigParser(ezdRpConfigRes.body)
        : null;
    if (ezdRpConfigParser) {
      ezdRpConfigSet[CONFIGKEY_URL_SSO] =
        ezdRpConfigParser.getConfig(CONFIGKEY_URL_SSO);
      ezdRpConfigSet[CONFIGKEY_VERSION] =
        ezdRpConfigParser.getConfig(CONFIGKEY_VERSION);
      ezdRpConfigSet[CONFIGKEY_APP_CONFIG] =
        ezdRpConfigParser.getConfig(CONFIGKEY_APP_CONFIG);
      ezdRpConfigSet[CONFIGKEY_URL_SSO] =
        ezdRpConfigParser.getConfig(CONFIGKEY_URL_SSO);
      ezdRpConfigSet[CONFIGKEY_URL_API] =
        ezdRpConfigParser.getConfig(CONFIGKEY_URL_API);
      ezdRpConfigSet[CONFIGKEY_URL_FILE] =
        ezdRpConfigParser.getConfig(CONFIGKEY_URL_FILE);
    }
  }
}
class _Config {
  ...
  getAppId() {
    return this._session.getConfig(
      this.getKey(CONFIGKEY_APP_CONFIG),
      ezdRpSessionConfigSection(),
    );
  }
  getEzdRpUrl(urlParams) {
    let ezdRpUrl = this._session.getConfig(
      this.getKey(CONFIGKEY_URL_EZDRP),
      ezdRpSessionConfigSection(),
    );
    return urlParams ? `${ezdRpUrl}?${urlParams}` : ezdRpUrl;
  }
  getAuthUrl(urlParams) {
    let authUrl = this._session.getConfig(
      this.getKey(CONFIGKEY_URL_SSO),
      ezdRpSessionConfigSection(),
    );
  ...
  getFileUrl(urlParams) {
    let authUrl = this._session.getConfig(
      this.getKey(CONFIGKEY_URL_FILE),
      ezdRpSessionConfigSection(),
    );
    return urlParams ? `${authUrl}?${urlParams}` : authUrl;
  }
  getApiUrl(urlParams) {
    let apiUrl = this._session.getConfig(
      this.getKey(CONFIGKEY_URL_API),
      ezdRpSessionConfigSection(),
    );
  ...
  getApiVersion() {
    return this._session.getConfig(
      this.getKey(CONFIGKEY_VERSION_API),
      ezdRpSessionConfigSection(),
    );
  }
  setApiVersion(ver) {
