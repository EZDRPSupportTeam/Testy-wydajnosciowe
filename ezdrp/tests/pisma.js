...
const ezdrp_metric_metadane = new Counter(
  "ezdrp_metrics_uzupelnianie_metadanych_kancelaria",
);
...
class _EzdRpPisma extends EzdRpTest {
  ...
  _dodajPismoByDokument(dokument, isRpw) {
    ...
    let pismoApiRes = this.checkBody(
      this.ezdRpApi.dodajPismo(dokumentyPisma, isRpw),
    );
    if (
      !pismoApiRes ||
      pismoApiRes.length === undefined ||
      pismoApiRes.length <= 0
    ) {
  ...
  szukajPismoZRpw(pisma, skip) {
    ...
    if (pisma && pisma.length > 0) {
      ...
      for (let pismo of pisma) {
        if (
          pismo.listaDokumentowPisma &&
          pismo.listaDokumentowPisma.length > 0
        ) {
          let isRpw = false;
          for (let zal of pismo.listaDokumentowPisma) {
            if (
              zal &&
              zal.dokumentNazwa &&
              zal.dokumentNazwa.includes("RPW/")
            ) {
              isRpw = true;
              break;
            } else {
  ...
  zapiszPismo(params) {
    return this.checkBody(
      this.ezdRpApi.zapiszPismo(params),
      null,
      null,
      null,
      ezdrp_metric_metadane,
    );
  }
