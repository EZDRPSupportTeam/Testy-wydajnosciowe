...
class _EzdRpStruktura extends EzdRpTest {
  ...
  pobierzDrzewo(param) {
    return this.checkBody(
      this.ezdRpApi.pobierzDrzewo(param),
      apiMetricCounterStruktura,
    );
  }
  szukaj(param) {
    return this.checkBody(
      this.ezdRpApi.szukaj(param),
      apiMetricCounterStruktura,
    );
  }
  getUsersExact(users, filter) {
    ...
    if (
      usersToFilter == null ||
      usersToFilter.length === undefined ||
  ...
  losujUzytkownika(struktura) {
    let losujZeStruktury = (komorka) => {
      ...
      const filtrUzytkownicy = komorka.uzytkownicy
        ? komorka.uzytkownicy.filter(
            (u) =>
              !u.imie.toLowerCase().includes("api") &&
              !u.email.toLowerCase().includes(currentUserLogin),
          )
        : [];
      const maUzytkownikow = filtrUzytkownicy.length > 0;
      const maKomorki =
        komorka.komorkiOrganizacyjne && komorka.komorkiOrganizacyjne.length > 0;
      ...
      if (maUzytkownikow && maKomorki) {
        ...
        if (isUser) {
          return filtrUzytkownicy[
            Math.floor(Math.random() * filtrUzytkownicy.length)
          ];
        } else {
          const nastepnaKomorka =
            komorka.komorkiOrganizacyjne.length > 1
              ? komorka.komorkiOrganizacyjne[
                  Math.floor(
                    Math.random() * komorka.komorkiOrganizacyjne.length,
                  )
                ]
        ...
        const nastepnaKomorka =
          komorka.komorkiOrganizacyjne.length > 1
            ? komorka.komorkiOrganizacyjne[
                Math.floor(Math.random() * komorka.komorkiOrganizacyjne.length)
              ]
            : komorka.komorkiOrganizacyjne[0];
        return losujZeStruktury(nastepnaKomorka);
      }
