...
class _EzdRpDashboard extends EzdRpTest {
  ...
  pobierzMenu(param) {
    return this.checkBody(this.ezdRpApi.pobierzMenu(param));
  }
  pobierzPraweMenu(idWidoku, idObiekt, idSekcja) {
    return this.checkBody(
      this.ezdRpApi.pobierzPraweMenu(idWidoku, idObiekt, idSekcja),
    );
  }
  ...
  pobierzLicznikNowychZadan() {
    return this.checkBody(this.ezdRpApi.pobierzLicznikNowychZadan());
  }
}
