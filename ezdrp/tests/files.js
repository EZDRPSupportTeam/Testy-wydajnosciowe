...
class _EzdRpFiles extends EzdRpTest {
    ...
    _dodajNowyDokumentNZ(params) {
        let dokumentApiRes = this.checkBody(this.ezdRpApi.dodajNowyDokumentNZ(params), null, null, null, ezdrp_metric_dodanie_dokumentu);
        if (!dokumentApiRes || dokumentApiRes.length === undefined || dokumentApiRes.length <= 0) {
            throw 'invalid dodaj nowy dokument response';
        }
        let dokumentRes = dokumentApiRes[0];
    ...
    directFileUpload(fileToUpload, fileToken) {
        let apiRes = this.checkBody(this.ezdRpApi.directFileUpload(fileToUpload, fileToken), null, null, null, null);
        return apiRes;
    }
    directFileChangeTitle(fileId, fileName, fileToken) {
        let apiRes = this.ezdRpApi.directFileChangeTitle(fileId, fileName, fileToken);
    }
