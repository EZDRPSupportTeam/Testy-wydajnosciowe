...
class _EzdRpProfile extends EzdRpTest {
  ...
  pobierzDostepneOperacje() {
    return this.checkBody(
      this.ezdRpApi.pobierzDostepneOperacje(),
      null,
      null,
      true,
    );
  ...
  _pobierzUzytkownika(param) {
    let uzytkownikResData = this.checkBody(
      this.ezdRpApi.pobierzUzytkownika(param),
    );
    let ssoUserId = this._session.getSsoUserId();
    if (
      uzytkownikResData &&
      uzytkownikResData.errorId &&
      uzytkownikResData.errorId !== ""
    ) {
  ...
  _pobierzJednostkeNadrzedna() {
    let jednstokaResData = this.checkBody(
      this.ezdRpApi.pobierzJednostkeNadrzedna(),
    );
  ...
  pobierzUstawienia(klucze) {
    return this.checkBody(
      this.ezdRpApi.pobierzUstawienia(klucze),
      null,
      null,
      true,
    );
  ...
  zaladujTozsamosc(doceloweStanowisko, doceloweBiuro) {
    ...
    if (
      doceloweStanowisko !== undefined &&
      doceloweStanowisko !== "" &&
      uzytkownikData !== null &&
      uzytkownikData.listaStanowisk !== null
    ) {
    ...
    if (
      stanowiskoToSelect === null &&
      uzytkownikData !== null &&
      uzytkownikData.listaStanowisk !== null &&
      uzytkownikData.listaStanowisk.length >= 1
    ) {
    ...
    if (stanowiskoToSelect != null && stanowiskoToSelect.idStanowisko !== "") {
      uzytkownikData = this._pobierzUzytkownika({
        idStanowiskoProponowane: stanowiskoToSelect.idStanowisko,
      });
      if (
        uzytkownikData !== null &&
        uzytkownikData.idStanowiska === stanowiskoToSelect.idStanowisko
      ) {
