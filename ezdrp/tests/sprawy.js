...
const ezdrp_metric_zakladanie_spraw = new Counter(
  "ezdrp_metrics_zakladanie_sprawy",
);

const apiMetricCounterZadanieDoAkceptacji = new Counter(
  "ezdrp_metrics_nowezadanie_doakceptacji",
);
const apiMetricCounterZadanieDoDekretacji = new Counter(
  "ezdrp_metrics_nowezadanie_dodekretacji",
);
const apiMetricCounterZadanieDoRealizacji = new Counter(
  "ezdrp_metrics_nowezadanie_dorealizacji",
);
const apiMetricCounterZadanieInne = new Counter(
  "ezdrp_metrics_nowezadanie_inne",
);
...
class _EzdRpSprawy extends EzdRpTest {
  ...
  dodajZadanie(params) {
    ...
    return this.checkBody(
      this.ezdRpApi.dodajZadanie(params),
      null,
      null,
      null,
      metricForZadanie,
  ...
  dodajNotatkeWlasna(idPrzestrzen, nrSprawy, idSprawa, trescNotatki) {
    return this.checkBody(
      this.ezdRpApi.dodajNotatkeWlasna({
        idPrzestrzen: idPrzestrzen,
        nrSprawy: nrSprawy,
        tresc: trescNotatki,
        idSprawa: idSprawa,
  ...
  pobierzHistorieSprawy(idSprawy) {
    return this.checkBody(
      this.ezdRpApi.pobierzHistorieSprawy({ idSprawa: idSprawy }),
    );
  }
  ...
  generujSprawe(param) {
    return this.checkBody(
      this.ezdRpApi.generujSprawe(param),
      null,
      null,
      null,
      ezdrp_metric_zakladanie_spraw,
    );
  }
  ...
  szukajSprawe(sprawy, skip, anyIfNot) {
    ...
    if (sprawy && sprawy.length > 0) {
      let skipCount = skip;
      if (skip >= sprawy.length && anyIfNot) {
        return sprawy.length > 0 ? sprawy[0] : null;
      }
