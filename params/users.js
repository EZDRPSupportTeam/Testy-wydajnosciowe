...
import { URL } from "../libs/k6-url/index.js";
...
const data = (function () {
  return new SharedArray("users.params", function () {
    ...
    let isRolesMappingExists =
      usersParam.roles_map !== undefined &&
      usersParam.roles_splitter !== undefined &&
      usersParam.roles_splitter !== "";
    let isUzytkownikStanowiskoMappingExists =
      usersParam.uzytkownik_stanowisko_rola_map !== undefined;
    ...
    let getUserRoles = function (l) {
      ...
      if (
        usersParam.roles_default !== undefined &&
        usersParam.roles_default.roles !== undefined &&
        (usersParam.roles_default.excluded_logins === undefined ||
          !usersParam.roles_default.excluded_logins.includes(l))
      ) {
    ...
    if (debugParams && debugParams !== "") {
      ...
      let firstUser = generatedUsersParam.users
        ? generatedUsersParam.users.at(0).login
        : "";
      let lastUser = generatedUsersParam.users
        ? generatedUsersParam.users.at(-1).login
        : "";
      log(
        `users count: ${
          generatedUsersParam.users ? generatedUsersParam.users.length : -1
        }, FIRST: ${firstUser}  LAST:${lastUser}`,
      );
    }
...
class _ParamsUsers {
  ...
  static getUserParams(input) {
    ...
    if (
      !inputData ||
      inputData.users === undefined ||
      inputData.users.length <= 0
    ) {
      throw "no users params data";
    }

    let forceUserList = SysConfig.getConfig("FORCE_USER_LIST");
    if (
      forceUserList &&
      forceUserList !== "" &&
      forceUserList !== "undefined"
    ) {
      let loginsList = forceUserList.split(",").map((l) => l.trim());
      let userIndex = userId % loginsList.length;
      let selectedLogin = loginsList[userIndex];
      let forcedUser = this.getUserByLogin(selectedLogin);
      if (forcedUser) {
        return forcedUser;
      } else {
        log(`WARNING: User ${selectedLogin} from FORCE_USER_LIST not found`);
      }
    }
