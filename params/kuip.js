  ...
  static prepareData(params) {
    ...
    if (params && params.length > 0) {
      ...
      for (let i = 0; i < podmiotyToPrepare.length; i++) {
        ...
        podmiot.nazwa = podmiotParam.podmiot_nazwa_format.replace(
          "#",
          podmiotRandom,
        );
        podmiot.symbol = podmiotParam.podmiot_symbol_format.replace(
          "#",
          podmiotRandom,
        );
        podmiot.epuap = podmiotParam.podmiot_epuap_format.replace(
          "#",
          podmiotRandom,
        );
        podmiot.numer = podmiotParam.numer_format.replace("#", podmiotRandom);
        podmiot.czy_aktualizowac =
          podmiotParam.podmiot_czy_istniejacy_aktualizowac;
        ...
        podmiot.struktura = this.prepareStruktura(
          podmiotParam.struktura,
          loginFormat,
          hasloFormat,
          podmiotRandom,
        );
        ...
        if (podmiotParam.uzytkownicy && podmiotParam.uzytkownicy.length > 0) {
          ...
          for (let n = 0; n < podmiotParam.uzytkownicy.length; n++) {
            ...
            uzytkownik.login = loginFormat
              .replace("#", podmiotRandom)
              .replace("%", uzytkownikParam.random);
            uzytkownik.haslo = hasloFormat
              .replace("#", podmiotRandom)
              .replace("%", uzytkownikParam.random);
  ...
  static prepareStruktura(struktura, loginFormat, hasloFormat, random) {
    ...
    for (let i = 0; i < struktura.length; i++) {
      ...
      if (dzial.struktura && dzial.struktura.length > 0) {
        newDzial.struktura = this.prepareStruktura(
          dzial.struktura,
          loginFormat,
          hasloFormat,
          random,
